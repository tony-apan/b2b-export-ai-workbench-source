---
title: "Mother Library Sync And Release Guide"
description: "区分私有母库源码同步、公开子库 Preview 和正式 Stable Release，避免把 Git 备份、公开预览与稳定发布混成同一个闸。"
type: "release-guide"
status: "Working"
owner: "AI"
created: "2026-07-28"
last_updated: "2026-07-30"
sources: ["Mother Library Private Source Manifest", "Sub-library contract", "Publishing and redaction policy", "Tony publish authorization 2026-07-30"]
...
|---|---|---|---|---|
| Private Source Sync | 私有 canonical 母库 | `repository_sync_status: Ready / Synced` | clean commit、基础验证、安全扫描、目标仓确认为 Private | 不授予公开许可证，不代表 Stable |
...
## Private Source Sync（当前母库采用）

母库 canonical 仓库是 `tony-apan/b2b-export-ai-workbench-source`，可见性必须为 **Private**。仓名不使用 `-private`；旧公开仓和旧私有仓保留原历史，不 force push、不覆盖。
...
5. 用 `gh repo view` 确认目标仓为 Private；
6. 只推送到 canonical private remote；
7. 从远端重新 clone 并复跑基础验证。

Private Source Sync 不需要把母库许可证改成开源许可证，也不使用 `build-mother-release.mjs --prepare`。母库 `release_status: BLOCK` 仅阻止**对外稳定发行**，不阻止已获用户授权的私有源码同步。

## 母库 release（正式公开导出，当前 BLOCK）

母库若未来需要公开，必须从 manifest allowlist 构建去敏 public-export artifact。普通构建：
...
子库必须使用自己的 `README.md`、`MANIFEST.md`、`RELEASE.md`、builder、validator 和独立仓库。母库 release 或私有同步都不会替子库完成：
...
- GitHub Private 只证明仓库可见性，不证明内容安全；
