...
`.github/workflows/validate-library.yml` 只做普通结构、working-tree 候选包和 adapter 验证，不承担正式 release，也不得读取正式 signer、approval、evidence 或 qualification log 配置。`.github/workflows/release-library.yml` 仅由 `workflow_dispatch` 启动，输入精确 `release_tag` 与外部 `approval_sidecar_base64`；dispatcher 只能提交尚未填充最终 evidence 字段的 approval intent，不能自行注入 evidence bundle。workflow checkout 对应 signed annotated tag，只运行 resolver 选中的一个 scope；pre-finalization 阶段只执行 structure/`--prepare` 检查，builder 输出精确 frozen candidate path。随后由 pinned trusted governance job 根据本次实际检查输出生成 `RELEASE-EVIDENCE.json`，再用其 canonical digest finalize approval sidecar；finalize 后才对同一 frozen candidate 使用外部 `RELEASE-APPROVAL.json` / `RELEASE-EVIDENCE.json` 执行 `--release`。trusted root `validate-release-approval.mjs` 把 approval/evidence 与实际 tag object、signer、canonical annotation、approval binding、commit tree 和 workflow/run 上下文精确绑定，归档后还会使用同一对 sidecar 重新验证提取树。isolated runtime job 通过短期 pinned artifact 把真实 TAP 日志传给 qualification job；母库写入明确的 `runtime_not_applicable` JSON。候选外 finalized approval、evidence 和 qualification check logs 与 archive/attestation 一起上传，不复制进 candidate。该本地结构不能伪装成远程 evidence、Protected Environment 或真人批准证明；远程 run、Environment reviewer、tag signer 身份和 attestation 仍需 GitHub 侧实证。
...
- 匹配 Environment 级 `FORMAL_RELEASE_TRUSTED_WORKFLOW_SHA` 与启用 secret `FORMAL_RELEASE_ENVIRONMENT_READY=release-governance-v1`；
- 配置 Environment secret `FORMAL_RELEASE_SIGNER_PUBLIC_KEY_BASE64`：值为正式 tag signer 的 ASCII-armored 或 binary public key 文件经过单行 base64 编码后的内容，只能包含公钥，不得包含 private/secret key。两个执行 `git verify-tag` 的 job 都会先把它解码到 `$RUNNER_TEMP`、创建权限收紧的临时 `GNUPGHOME`、静默 `gpg --import` 并要求至少解析出一个完整 fingerprint；缺失、base64 非法、导入失败或无 fingerprint 一律 BLOCK，key bytes 不写日志；
- 验证 signed annotated tag、candidate HEAD 和可信 GPG signing-key fingerprint；Environment variable `FORMAL_RELEASE_TRUSTED_TAG_SIGNERS` 继续登记 `VALIDSIG` 实际返回的 signing subkey 完整 fingerprint 并做逐字 allowlist，导入公钥本身不授予信任，也不替代 fingerprint allowlist、轮换和撤销；
- 比对母库 scripts、release workflow、`sub-libraries/registry.json` 与目标子库 scripts，阻止 candidate 自带弱化 validator；
- 在独立 runner job 对目标子库运行可信 runtime-test profile：`verify-runtime-test-profile.mjs` 逐级拒绝 adapter/祖先 symlink，要求 package/lock/test 与祖先目录 npm/package-manager 控制文件和 trusted governance 一致，并把显式 allowlist 中的 candidate implementation 与 trusted tests 复制到 `$RUNNER_TEMP/runtime-subject`。依赖安装与测试在 `FORMAL_RELEASE_NODE20_IMAGE_DIGEST` 固定的 Node 容器内运行；测试容器只读挂载 subject、禁网、降权、移除 capabilities，不能看到 sibling governance checkout 或 GitHub command files。pinned workflow 直接执行固定 `node --test --test-reporter=tap`，`verify-node-test-summary.mjs` 再要求精确的最终 test plan/pass/fail/skipped 计数；当前 `website-content-ops` trusted profile 固定为 4 个测试文件和 160/160 全通过（媒体 47、正文图片 52、正文格式 13、文章生命周期/taxonomy 48），旧 120、131、136、145、156、158、159、161 或任意 failed/skipped 都必须 fail-closed。完整 11 文件开发回归另有 `runtime-test-plan.json` 作为唯一真源（当前 267/267），不得与 160 正式 profile 混用。runtime job 输出实际测试的 candidate commit 与 annotated tag object SHA，qualification job 的全新 checkout 通过 `verify-tested-candidate-identity.mjs` 精确比对后再构建，避免 candidate 重定向测试对象、污染后续 builder、替换 npm lifecycle、提前退出伪造空跑，或 tag 在两个 job 之间 retarget；未来新增子库若没有 profile，正式 qualification 必须 BLOCK；
