...
function validateEvidenceChecks(checks, profile, manifest, sourceCommit, contentDigest, tagName, tagBinding = null) {
  ...
  for (const id of requiredIds) {
    ...
    if (id === 'governance-tests' || id === 'runtime-tests') {
      ...
      if (id === 'runtime-tests') {
        if (expected !== EXPECTED_RUNTIME_TESTS) fail(`approval evidence runtime-tests expected_tests must be ${EXPECTED_RUNTIME_TESTS}`);
...
function outputLineCount(buffer) {
  if (!buffer.length) return 0;
  const text = buffer.toString('utf8');
  return text.split('\n').length - (text.endsWith('\n') ? 1 : 0);
}
function trustedEnv(name, pattern = null) {
  const value = (process.env[name] ?? '').trim();
  if (!value || (pattern && !pattern.test(value))) fail(`${name} is required and must be valid in trusted evidence mode`);
  return value;
}
function validateTrustedEvidenceChecks(evidence, scopeKind, manifest, sourceCommit, contentDigest, tagName, tagBinding, evidencePath) {
  const contract = trustedCheckContract(scopeKind);
  const expectedIds = [...contract.keys()];
  const checks = evidence?.checks;
  if (!Array.isArray(checks)) { fail('trusted evidence checks must be an array'); return; }
  const actualIds = checks.map((check) => check?.id);
  if (JSON.stringify(actualIds) !== JSON.stringify(expectedIds)) fail(`trusted evidence check order/set must exactly equal ${expectedIds.join(', ')}`);

  const execution = evidence?.execution;
  exactKeys(execution, ['source', 'workflow_sha', 'run_id', 'run_attempt', 'job_id', 'runtime_job_id', 'candidate_commit', 'tag_name', 'tag_object_sha'], 'evidence.execution');
  const expectedExecution = {
    source: TRUSTED_EVIDENCE_SOURCE,
    workflow_sha: trustedEnv('RELEASE_ACTUAL_WORKFLOW_SHA', /^[a-f0-9]{40}$/),
    run_id: trustedEnv('RELEASE_ACTUAL_RUN_ID', /^\d+$/),
    run_attempt: Number(trustedEnv('RELEASE_ACTUAL_RUN_ATTEMPT', /^\d+$/)),
    job_id: trustedEnv('RELEASE_ACTUAL_JOB_ID', /^[a-z0-9][a-z0-9-]*$/),
    runtime_job_id: (process.env.RELEASE_ACTUAL_RUNTIME_JOB_ID ?? 'isolated-runtime-tests').trim(),
    candidate_commit: trustedEnv('RELEASE_ACTUAL_CANDIDATE_COMMIT', /^[a-f0-9]{40}$/),
    tag_name: trustedEnv('RELEASE_ACTUAL_TAG_NAME'),
    tag_object_sha: trustedEnv('RELEASE_ACTUAL_TAG_OBJECT_SHA', /^[a-f0-9]{40}$/),
  };
  if (canonicalJson(execution, 'evidence.execution') !== canonicalJson(expectedExecution, 'expected trusted execution')) fail('trusted evidence execution context does not exactly match workflow/run/job/candidate/tag context');
  if (expectedExecution.candidate_commit !== sourceCommit || expectedExecution.tag_name !== tagName) fail('trusted execution candidate commit or tag name does not match approval/candidate');
  const contextSha256 = createHash('sha256').update(JSON.stringify(execution, Object.keys(execution ?? {}).sort()), 'utf8').digest('hex');
  const evidenceRoot = dirname(evidencePath);
  const governanceRoot = resolve(trustedEnv('RELEASE_TRUSTED_GOVERNANCE_ROOT'));
  if (!existsSync(governanceRoot) || !statSync(governanceRoot).isDirectory()) fail('RELEASE_TRUSTED_GOVERNANCE_ROOT must point to trusted governance code');
  let governancePlan = [];
  const planRun = spawnSync(process.execPath, [join(governanceRoot, 'scripts/run-governance-tests.mjs'), '--list'], { cwd: governanceRoot, encoding: 'utf8' });
  if (planRun.status !== 0) fail('trusted governance test plan could not be resolved from trusted governance code');
  else governancePlan = planRun.stdout.trim().split('\n').filter(Boolean).map((line) => line.split('\t')[0]);

  for (const [index, check] of checks.entries()) {
    if (!check || typeof check !== 'object' || Array.isArray(check)) { fail(`trusted evidence checks[${index}] must be an object`); continue; }
    exactKeys(check, ['id', 'status', 'result'], `trusted evidence checks[${index}]`);
    const id = check.id;
    if (check.status !== 'pass') fail(`trusted evidence check ${id ?? index} must have status pass`);
    const result = check.result;
    if (!result || typeof result !== 'object' || Array.isArray(result)) { fail(`trusted evidence check ${id}.result must be an object`); continue; }
    const base = ['schema', 'command', 'exit_code', 'output_file', 'output_sha256', 'output_bytes', 'output_lines', 'context_sha256', 'producer_job'];
    const extra = id === 'governance-tests' ? ['test_plan', 'expected_tests', 'passed_tests', 'failed_tests', 'skipped_tests']
      : id === 'runtime-tests' ? ['test_plan', 'expected_tests', 'passed_tests', 'failed_tests', 'skipped_tests', 'image_digest', 'status', 'reason']
        : id === 'runtime-applicability' ? ['applicability', 'contract_path', 'contract_present', 'status', 'reason']
          : id === 'artifact-validation' ? ['mode', 'content_digest']
            : id === 'commit-provenance' ? ['source_commit', 'selected_file_count', 'commit_bound_file_count', 'unbound_file_count', 'missing_commit_file_count']
              : id === 'tag-signature' ? ['tag_name', 'target_commit', 'tag_object_sha', 'signer_fingerprint', 'signature_status', 'annotation_schema', 'annotation_sha256', 'approval_binding_digest_algorithm', 'approval_binding_sha256', 'approval_id', 'scope_kind', 'scope_id', 'version', 'candidate_content_digest']
                : ['mode'];
    exactKeys(result, [...base, ...extra], `trusted evidence check ${id}.result`);
    if (result.schema !== TRUSTED_RESULT_SCHEMA) fail(`trusted evidence check ${id} result.schema must be ${TRUSTED_RESULT_SCHEMA}`);
    if (result.command !== contract.get(id)) fail(`trusted evidence check ${id} command does not match the fixed contract`);
    if (result.exit_code !== 0) fail(`trusted evidence check ${id} exit_code must be 0`);
    hex(result.output_sha256, `trusted evidence check ${id}.result.output_sha256`);
    integerAtLeast(result.output_bytes, `trusted evidence check ${id}.result.output_bytes`);
    integerAtLeast(result.output_lines, `trusted evidence check ${id}.result.output_lines`);
    if (result.context_sha256 !== contextSha256) fail(`trusted evidence check ${id} context_sha256 does not bind the exact execution context`);
    const expectedProducer = id === 'runtime-tests' ? expectedExecution.runtime_job_id : expectedExecution.job_id;
    if (result.producer_job !== expectedProducer) fail(`trusted evidence check ${id} producer_job must be ${expectedProducer}`);

    if (id === 'runtime-tests') {
      if (result.output_file !== null) fail('trusted runtime-tests output_file must be null because output is bound from the isolated job');
      const runtimeSha = trustedEnv('RELEASE_ACTUAL_RUNTIME_OUTPUT_SHA256', /^[a-f0-9]{64}$/);
      const runtimeBytes = Number(trustedEnv('RELEASE_ACTUAL_RUNTIME_OUTPUT_BYTES', /^\d+$/));
      const runtimeLines = Number(trustedEnv('RELEASE_ACTUAL_RUNTIME_OUTPUT_LINES', /^\d+$/));
      if (result.output_sha256 !== runtimeSha || result.output_bytes !== runtimeBytes || result.output_lines !== runtimeLines) fail('trusted runtime-tests output receipt does not match isolated runtime job outputs');
    } else {
      const outputFile = portableRelativePath(result.output_file, `trusted evidence check ${id}.result.output_file`);
      if (outputFile && !outputFile.startsWith('checks/')) fail(`trusted evidence check ${id} output_file must remain under checks/`);
      const outputPath = outputFile ? resolve(evidenceRoot, outputFile) : '';
      if (outputPath && !outputPath.startsWith(`${resolve(evidenceRoot)}${process.platform === 'win32' ? '\\' : '/'}`)) fail(`trusted evidence check ${id} output_file escapes the evidence root`);
      if (!outputPath || !existsSync(outputPath) || !statSync(outputPath).isFile()) fail(`trusted evidence check ${id} output log is missing`);
      else {
        const bytes = readFileSync(outputPath);
        if (createHash('sha256').update(bytes).digest('hex') !== result.output_sha256 || bytes.length !== result.output_bytes || outputLineCount(bytes) !== result.output_lines) fail(`trusted evidence check ${id} output receipt does not match the stored log bytes`);
      }
    }

    if (id === 'governance-tests') {
      if (JSON.stringify(result.test_plan) !== JSON.stringify(governancePlan)) fail('trusted governance test_plan does not match trusted run-governance-tests.mjs --list');
      if (result.expected_tests !== governancePlan.length || result.passed_tests !== governancePlan.length || result.failed_tests !== 0 || result.skipped_tests !== 0) fail('trusted governance test counts must exactly match the fixed plan with all tests passing');
    } else if (id === 'runtime-tests') {
      if (JSON.stringify(result.test_plan) !== JSON.stringify(EXPECTED_RUNTIME_TEST_PLAN) || result.expected_tests !== EXPECTED_RUNTIME_TESTS || result.passed_tests !== EXPECTED_RUNTIME_TESTS || result.failed_tests !== 0 || result.skipped_tests !== 0) fail('trusted runtime-tests must exactly match the fixed all-pass profile');
      if (result.image_digest !== trustedEnv('RELEASE_ACTUAL_RUNTIME_IMAGE_DIGEST', /^sha256:[a-f0-9]{64}$/) || result.status !== 'runtime_verified' || result.reason !== SUB_LIBRARY_RUNTIME_REASON) fail('trusted runtime-tests image/status/reason does not match the isolated runtime job contract');
    } else if (id === 'runtime-applicability') {
      if (scopeKind !== 'mother-library' || result.applicability !== 'none' || result.contract_path !== null || result.contract_present !== false || result.status !== 'runtime_not_applicable' || result.reason !== MOTHER_RUNTIME_REASON) fail('mother runtime-applicability evidence must bind the exact machine-readable N/A contract');
      if (manifest.runtime_applicability !== 'none' || manifest.runtime_contract !== null || manifest.files?.includes('RUNTIME-CONTRACT.json') || existsSync(join(candidateRoot, 'RUNTIME-CONTRACT.json'))) fail('mother runtime N/A is invalid when the frozen candidate contract or actual content declares a runtime');
    } else if (id === 'artifact-validation') {
      if (result.mode !== 'prepare' || result.content_digest !== contentDigest) fail('trusted artifact-validation must bind prepare mode and the exact candidate content digest');
    } else if (id === 'commit-provenance') {
      if (result.source_commit !== sourceCommit || result.selected_file_count !== manifest.files?.length || result.commit_bound_file_count !== manifest.files?.length || result.unbound_file_count !== 0 || result.missing_commit_file_count !== 0) fail('trusted commit-provenance result must bind every selected candidate file to the source commit');
    } else if (id === 'tag-signature') {
      if (result.tag_name !== tagName || result.target_commit !== sourceCommit || result.tag_object_sha !== expectedExecution.tag_object_sha || result.signer_fingerprint !== tagBinding?.signerFingerprint || result.signature_status !== 'trusted') fail('trusted tag-signature result does not match the actual approval/tag context');
      if (!tagBinding || result.annotation_schema !== TAG_ANNOTATION_SCHEMA || result.annotation_sha256 !== tagBinding.annotationSha256 || result.approval_binding_digest_algorithm !== APPROVAL_BINDING_ALGORITHM || result.approval_binding_sha256 !== tagBinding.approvalBindingSha256 || result.approval_id !== tagBinding.annotation.approval_id || result.scope_kind !== tagBinding.annotation.scope?.kind || result.scope_id !== tagBinding.annotation.scope?.id || result.version !== tagBinding.annotation.version || result.candidate_content_digest !== tagBinding.annotation.candidate_content_digest) fail('trusted tag-signature result does not bind the canonical approval annotation');
    } else {
      const expectedMode = id === 'index-validation' ? 'strict' : id === 'document-id-validation' && scopeKind === 'sub-library' ? 'sub-library-scope' : id === 'document-id-validation' ? 'default' : 'release';
      if (result.mode !== expectedMode) fail(`trusted evidence check ${id} mode must be ${expectedMode}`);
    }
  }
}
...
function validateCommitProvenance(manifest, root, sourceCommit) {
  ...
  const expectedSourceScope = manifest.release_scope === 'standalone-mother-library'
    ? 'repository-root'
    : `sub-libraries/${manifest.package_id ?? ''}`;
  const sourceScope = portableRelativePath(manifest.source_scope, 'candidate.source_scope');
  if (sourceScope && sourceScope !== expectedSourceScope) fail(`candidate source_scope must be ${expectedSourceScope} for ${scopeLabel} approval`);

  const provenance = manifest.source_provenance ?? {};
  if (!provenance || typeof provenance !== 'object' || Array.isArray(provenance)) {
    fail(`candidate source_provenance must be an object for ${scopeLabel} approval`);
    return [];
  }
  ...
  if (manifest.release_scope === 'standalone-sub-library' && provenance.source_scope !== expectedSourceScope) fail(`candidate source_provenance.source_scope must be ${expectedSourceScope}`);
  ...
  for (const [index, record] of provenance.files.entries()) {
    ...
    const expectedRepositoryPath = expectedSourceScope === 'repository-root' ? path : `${expectedSourceScope}/${path}`;
    if (manifest.release_scope === 'standalone-sub-library') {
      ...
      if (repositoryPath && repositoryPath !== expectedRepositoryPath) fail(`candidate source_provenance file ${path} repository_path must be ${expectedRepositoryPath}`);
...
const requireTrustedEvidence = process.env.RELEASE_REQUIRE_TRUSTED_EVIDENCE === '1';
const expectedProfile = requireTrustedEvidence
  ? trustedProfile(expectedScopeKind)
  : expectedScopeKind === 'mother-library'
    ? 'mother-release-v1'
    : expectedScopeKind === 'sub-library'
      ? 'sub-library-release-v1'
      : '';
...
if (evidenceDigestAlgorithm !== EVIDENCE_DIGEST_ALGORITHM) fail(`approval validation.evidence_digest_algorithm must be ${EVIDENCE_DIGEST_ALGORITHM}`);
if (requireTrustedEvidence && validation.evidence_source !== TRUSTED_EVIDENCE_SOURCE) fail(`approval validation.evidence_source must be ${TRUSTED_EVIDENCE_SOURCE} in trusted evidence mode`);
...
const validationCompletedAt = isoTimestamp(validation.completed_at, 'validation.completed_at');
...
const expectedEvidenceSchema = requireTrustedEvidence ? TRUSTED_EVIDENCE_SCHEMA : 'release-evidence/v1';
if (evidence.schema !== expectedEvidenceSchema) fail(`approval evidence schema must be ${expectedEvidenceSchema}, got ${evidence.schema ?? 'missing'}`);
...
const approvedAt = isoTimestamp(humanApproval.approved_at, 'approval.approved_at');
if (Number.isFinite(approvedAt) && Number.isFinite(validationCompletedAt) && approvedAt > validationCompletedAt) fail('approval approved_at must not be later than validation.completed_at');
...
if (requireTrustedEvidence) {
  validateTrustedEvidenceChecks(
    evidence,
    expectedScopeKind,
    manifest,
    sourceCommit,
...
if (failures.length) {
  ...
  console.log('\nAPPROVAL_RECORD_PASS: the supplied approval record is bound to this exact candidate digest, canonical evidence bundle, and tag namespace.');
}
