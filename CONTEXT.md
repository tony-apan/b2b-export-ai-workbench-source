...
- **身份**：根目录是逻辑母库；`sub-libraries/` 是独立发布子库；tracked 母库是普通用户只读的上游核心，真实运行数据只进根目录下 Git 隔离的 `customer-runtime/` 或外部挂载的客户私有运行区。
...
- **客户运行**：[仓库内多客户代运营运行模型](wiki/00_meta/in-repository-agency-runtime-model.md)；客户搜索必须先锁定 `client_id`，日志不能替代任务当前状态。
