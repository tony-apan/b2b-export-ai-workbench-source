...
|---|---|---|---|---|---|---|---|
| [Website Content Operations](website-content-ops/README.md) | 从公司/产品/客户语言到内容、图片、CMS 草稿、验证和写回 | `human-playbook` + `ai-skill-draft` + `toolkit` | Validated | historical `0.3.2-preview.1`; current `website-content-ops-0.4.0-preview.1` / version `0.4.0-preview.1` | [README](website-content-ops/README.md) | 一键安装+自测：`python3 website-content-ops/SKILL-INSTALL/install.py`（Windows 用 `install.cmd`）；自动 `npm ci` 并跑完整 11 文件自测套件 | `Preview`（2026-09-03）：许可 cleared、三张 source card 逐卡 clearance、当前候选 `0.4.0-preview.1` 已随公开仓 main `4ccab49` 发布（`preview_publication_status: Published`）；非 Stable、生产动作需批准；见 [MANIFEST](website-content-ops/MANIFEST.md) |
