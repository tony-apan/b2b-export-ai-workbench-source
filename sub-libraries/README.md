...
|---|---|---|---|---|---|---|---|
| [Website Content Operations](website-content-ops/README.md) | 从公司/产品/客户语言到内容、图片、CMS 草稿、验证和写回 | `human-playbook` + `ai-skill-draft` + `toolkit` | Validated | historical `0.3.2-preview.1`; current `website-content-ops-0.4.0-preview.2` / version `0.4.0-preview.2` | [README](website-content-ops/README.md) | 一键安装+自测：`python3 website-content-ops/SKILL-INSTALL/install.py`（Windows 用 `install.cmd`）；自动 `npm ci` 并跑完整 11 文件自测套件 | `Preview` candidate `0.4.0-preview.2` is Ready for Public Preview publication; non-Stable; production actions require approval; see [MANIFEST](website-content-ops/MANIFEST.md) |
