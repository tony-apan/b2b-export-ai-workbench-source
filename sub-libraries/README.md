...
|---|---|---|---|---|---|---|---|
| [Website Content Operations](website-content-ops/README.md) | 从公司/产品/客户语言到内容、图片、CMS 草稿、验证和写回 | `human-playbook` + `ai-skill-draft` + `toolkit` | Validated | historical `0.3.2-preview.1`; current `unassigned` / version `null`（dirty working tree，未发布候选） | [README](website-content-ops/README.md) | 一键安装+自测：`python3 website-content-ops/SKILL-INSTALL/install.py`（Windows 用 `install.cmd`）；自动 `npm ci` 并跑完整 11 文件自测套件 | `BLOCK`（2026-09-07 ISS-141 回退）：历史公开版仅 `0.3.2-preview.1`（2026-07-30）；母库裸 tag `v0.4.0-preview.1` 为非规范事故证据（wrong namespace + 一版三绑），0.3.2 后全部改动是未发布候选；许可 cleared（三张 source card 逐卡闭合）；下一候选 `0.4.0-preview.2` 用 namespaced tag `sub-library/website-content-ops/v0.4.0-preview.2`；Stable 继续阻断；见 [MANIFEST](website-content-ops/MANIFEST.md) |
