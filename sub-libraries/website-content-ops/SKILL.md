...
19. **域名绑定（建站后做，2026-09-09 实测，ISS-147）**（完整指南：[DOMAIN-SETUP.md](TOOLS/interface-kit/DOMAIN-SETUP.md)）：`domain-check.py` 巡检**五项**（平台侧 + 本地 dig 双向对账 + **SSL 双层检测**，**不需要代理**）：① 已添加域名 ② @ 与 www 都绑定 ③ NS 服务商 ④ CNAME 实际指向 == `runtime_site_domain` ⑤ SSL（本地 `probe_tls` 实测握手+证书校验，`probe_http` 验证线上可达且**落点须同站**）+ 与平台状态三方交叉校验。**关键平台限制**：Cloudflare **根域 CNAME 强制展平且无法关闭**（官方文档明示），CF 用户无法用根域通过 EdgeOne 验证 → 给 CF 客户的标准方案是 **www 为主 + 根域 301**，或建议迁 DNS 到阿里云（根域 CNAME + MX 实测可共存）。绑定用 `api.add_domain(..., authorization_confirmed=True)`、状态同步用 `api.refresh_domain()`；删除/改主域带授权闸（delete 另需 confirm_token=域名）。**证书长期卡 `failed` 时**可用 `api.rebind_domain(..., local_tls_ok=False)` 解绑重绑重新触发签发（实测 failed→requested→active），但**会重置 EdgeOne 配置致域名中断约 100 秒**——仅当**本地 SSL 也确实失败**时用（本地正常=平台滞后，重绑是自伤，工具三道门会拒绝：授权闸/主域名/本地正常）。**给客户的话术在 [client-input-checklist.md](TOOLS/interface-kit/templates/client-input-checklist.md) 〇-c**（购买引导/绑定确认/按 NS 商 DNS 指引/验证反馈），时机=**建站完成客户满意之后**，不是开工前置。

以上均已写入 `TOOLS/interface-kit/index/issues.tsv`（ISS-105/106/107/108/120..129、131、134..139、142..147）、`MODULES.md`（网格规则块）、`RUNBOOK-ANYONE.md`（§2 事实表 + §2.1 诊断树 + §8.1 执行路径决策树 + §8.2 demo 残留词库 + §8.3 自修复引导）与 `NEW-SITE-ONEPASS.md` 步骤 3/5/6/7/9/10 + 末尾卡住时清单。
