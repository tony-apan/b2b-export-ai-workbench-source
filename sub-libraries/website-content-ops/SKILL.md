...
19. **域名绑定（建站后做，2026-09-09 实测，ISS-147）**（完整指南：[DOMAIN-SETUP.md](TOOLS/interface-kit/DOMAIN-SETUP.md)）：`domain-check.py` 巡检（平台侧 + 本地 dig 双向对账，**不需要代理**）四项：① 已添加域名 ② @ 与 www 都绑定 ③ NS 服务商 ④ CNAME 实际指向 == `runtime_site_domain`。**关键平台限制**：Cloudflare **根域 CNAME 强制展平且无法关闭**（官方文档明示），CF 用户无法用根域通过 EdgeOne 验证 → 给 CF 客户的标准方案是 **www 为主 + 根域 301**，或建议迁 DNS 到阿里云（根域 CNAME 可保留，实测 laifaxin.com）。绑定用 `api.add_domain(..., authorization_confirmed=True)`、状态同步用 `api.refresh_domain()`；删除/改主域带授权闸（delete 另需 confirm_token=域名）。**给客户的话术在 [client-input-checklist.md](TOOLS/interface-kit/templates/client-input-checklist.md) 〇-c**（购买引导/绑定确认/按 NS 商 DNS 指引/验证反馈），时机=**建站完成客户满意之后**，不是开工前置。
