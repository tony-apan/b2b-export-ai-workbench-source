...
⑤ 创建/更新  最终全字段 payload → 独立 READY record + fresh capability → create 走 full-source JS Controller（article:create handler + 三真实 provider，缺 provider 本次 BLOCK；Python fail-closed）→ mutate_reviewed_post(exact ID) → audit
```
