...
> 生成时间：2026-08-30｜查询：`python3 registry_tools.py find <词>`｜更新后跑 `verify` + `gen`
...
| SCRIPT-003 | script | 建站流水线工具 | `../site_pipeline.py` | current | validate(含 3/4 数量门)/generate/check(模板词)/diff(readback)/gate(一键门)/contact(demo 联系方式门)/audit(13 项一体对抗审计+JSON 报告：FAQ答案SSR/真实CTA/单位/绝对化/Markdown/lang/canonical/JSON-LD/图片属性) |
...
| CAN-004 | doc | 文章页前端 SEO 合同（ID-0002） | `../../../../../../../sub-libraries/website-content-ops/PLAYBOOKS/id-0002-article-page-frontend-seo-contract.md` | current | 索引资格/文档语言/canonical/结构化数据/OG/语义 HTML/图片合同 |
...
| SCRIPT-007 | script | 友商监控工具 | `../../../../../river-trail/40_competitors/competitor-watch.py` | current | 每友商一目录：add(初始化 sitemap.tsv+snapshots)/scan(对比输出 new/updated/removed)/watch(批量)/report；TSV 主表+md profile |
...
| DOC-006 | doc | 友商自动发现流程 | `../../../../../river-trail/40_competitors/COMPETITOR-DISCOVERY.md` | current | daily 搜索语法+domains.tsv 去重表+探 sitemap+promote/scan 闭环；发现用 web_search 脚本登记 |
| TPL-016 | template | 站点登记表（多公司多网站） | `../../../../../river-trail/site-registry.tsv` | current | company→sites 映射：每品牌一 AllinCMS 站；sites/<slug>/ 目录层约定 |
| EV-010 | evidence | 双审记录（SOL+TERRA） | `../../../../../river-trail/70_evidence/DUAL-REVIEW-20260829.md` | current | 双审机制+本轮执行状态+客观门结果（路径/编译/列一致/凭据0/超时/上手）+调优 |
...
| ISS-012 | boundary | platform | 首页/列表页无 og 与 canonical；全站无 JSON-LD | 平台模板行为（详情页由平台按记录注入 og） | 接受；详情页 og 可用；不阻塞交付 | 不当作内容 bug | DOC-003 |
...
| ISS-029 | fixed | double-review | TERRA P0(上手): 查询命令路径不存在（00_shared 无 interface-kit 分发）；任务目录为空（无 TASK/HANDOFF） | 权威副本未分发；任务真源只留在 thermal-flask 任务目录 | interface-kit 复制到 customer-runtime/00_shared/；river-trail/30_tasks/river-trail-full-run-20260829/ 补 TASK.json+HANDOFF.md；README 指针修正 | 新客户先分发 00_shared 再写 README | DOC-002 |
...
| ISS-031 | fixed | outsider-review | 旁观审发现：HANDOFF/README 计数过期（49→51、18→27、25→33）与快速入口路径错误（interface-kit/PLAYBOOKS 不存在） | 回填后未刷新计数；路径写错层级 | 计数改“以 verify/ls 为准”；验收合同补全仓根路径 sub-libraries/website-content-ops/PLAYBOOKS/id-0007；wiki 命令注明仅 river-trail 可用 | 任何文档写具体数字/路径前核对其来源 | HANDOFF.md |
...
| ISS-033 | boundary | platform | 表单提交 submitRuntimeFormAction 服务端 500（digest 753112626）——平台缺陷确认 | 已用真实邮箱(xmc1204@qq.com)+浏览器同源 fetch+精确 payload 三盲测仍 500；排除 API 用法/邮箱因素 | 向平台支持反馈；等待修复或使用替代询盘通道 | 任何站的表单均可能受影响——上线前实提一次（ID-0007 D-7），失败则记录平台缺陷 | CAN-001 |
...
| ISS-035 | fixed | contact | 联系方式必须以用户提供为准并全站替换（8617091913071 示例：电话+WhatsApp） | demo 值散布 contact 页/首页/全站 globals 浮钮 | 用户提供→替换→contact 门扫描验证（contact-scan）→未提供项列入交付清单 | 联系方式必填收集；替换后 contact 门必跑 | TPL-009 |
...
| ISS-043 | boundary | platform | html lang 硬编码 zh-CN（英文站）——第三方评审 P0 实锤为平台 root layout 硬编码 | 平台无语言字段（site/theme 均无 lang/locale） | 记录+向平台反馈；内容层无法修 | 不作站点可修项；语言类 SEO 归平台 | ID-0007 A6 |
...
| ISS-053 | boundary | platform | img 无 width/height/srcset/loading 属性（CLS/移动端风险） | 平台图片输出无尺寸与加载策略属性 | 记录平台 BLOCK；首图已有 preload；页面对比属性缺失记录 | 图片属性项归平台，不参与站内可修判定 | ID-0007 C8 |
...
| ISS-058 | fixed | verification | 审计仅人工/靠文档提醒，可重复出现'文档说完成但前端未落地' | 缺自动化机检门；人工清单靠自觉 | site_pipeline.py audit 门（14 项：FAQ答案SSR/CTA真链/单位/绝对化/Markdown/lang/canonical/JSON-LD/img 属性+机器可读 JSON 报告）接入 ONBOARDING 2.9/ID-0007 B7/OUTSIDER §3.0 | 任何上线前必跑 audit；评审回应前先 audit 拿事实 | DOC-002 |
...
| ISS-068 | fixed | seo-copy | Example 残留 demo 内容 4 处（对抗补充扫描发现）：post-detail 页 related 块 'More from the journal'；posts 页 page-header 'Journal/Product care and ideas' 与 recommendations 'featured in our journal'；social-floating-button WhatsApp 空号按钮（zod 空串回默认 wa.me/447762109411） | 早期 demo 清理只扫了 TEMPLATE_WORDS 旧词表 + 首页/关于/联系页，未覆盖文章详情页/posts 页块 props 与全局浮动按钮；zod default 回填机制未识破 | post-detail 加 material-story-split CTA 真链接(/contact-us?source=example-article) + related 文案换 Example；posts 页两块文案全换；浮动按钮整体移除（children+elements 同删，置空无效）；TEMPLATE_WORDS/DEMO_CONTACTS 补 13 个漏网 demo 词与 wa.me/447762109411 | demo 清理必须覆盖全部 7 页 + 全局块（header/footer/dialog/floating）+ 详情页块 props；'置空'对 zod default 字段无效，移除=删元素；模板词表每遇漏网立即扩充 | RUNBOOK-ANYONE §2/§9; MODULES |
