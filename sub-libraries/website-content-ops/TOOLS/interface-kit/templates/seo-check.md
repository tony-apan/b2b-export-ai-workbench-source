...
> 平台边界见 ID-0007 C 表（og/canonical/JSON-LD 缺失属平台行为，不误报）。本清单检查**内容侧可控项**。
...
- [ ] lang/viewport 正确；slug 短可读（- 分隔，无时间戳/无中文）
...
## 注意（平台边界，不阻塞）
- canonical：全站缺失（平台行为）；首页/列表页无 og（平台行为）
- JSON-LD：无（平台行为）；如未来平台支持再补 Article/Product/Breadcrumb
