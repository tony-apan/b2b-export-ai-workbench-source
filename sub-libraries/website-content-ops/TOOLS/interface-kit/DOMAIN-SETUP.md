...
description: "规定给客户绑定自定义域名的完整流程与原理：CNAME 机制、Cloudflare 与阿里云分步实战、代理状态互斥陷阱（www 灰云 / 根域橙云）、根域在 CF 被强制展平的实测证据与 301 解法、接口自动化、巡检排查、域名数量上限、根域在各 DNS 服务商的行为差异（CF 强制展平 / 阿里云可共存）等边界，以及给客户的话术。"
type: "doc"
status: "Working"
owner: "AI"
created: "2026-09-15"
...
> 完全不懂 DNS 也能照着走。每步都有"AI 做什么 / 客户做什么 / 怎么确认成功"。

| 步 | AI 做什么 | 客户做什么 | 成功信号 |
|---|---|---|---|
| **1** | 问客户要域名（或确认没有） | 报一个域名，或说"没有" | 拿到域名 |
| **2** | **跑绑前体检**：`domain-check.py <slug> --check-domain <域名>` | 等待（几秒） | 看到"已注册 + 可绑定" |
| **3** | 未注册 → 给购买引导；已注册 → 确认"是你自己买的吗" | 买域名 / 确认归属 | 域名在手且确认归属 |
| **4** | `add_domain` 绑定 + 把该 DNS 商的操作步骤发给客户 | 照步骤加 CNAME 记录（3 分钟） | 平台 `cnameStatus: active` |
| **5** | `refresh_domain` 刷新，等平台签证书 | —— | `certificateStatus: active` |
| **6** | 跑巡检 `domain-check.py <slug>`，把网址发给客户验收 | 打开网址看是否正常 | 巡检 0 须修 + 浏览器能打开 |

**第 2 步的体检结果决定后面怎么走**（这是新手最容易懵的地方）：

| 体检结果 | 含义 | 下一步 |
|---|---|---|
| ❌ 未注册 | 域名还没被买 | 走购买引导（约 10 美元或 85 元/年） |
| ✅ 已注册 + 有归属确认 | 正常 | 直接绑定 + 配 DNS |
| ⚠️ 快到期 | 30 天内到期 | 先让客户续费（过期网站会挂） |
| ❓ 查不到 | 拼写可能错 | 让客户核对拼写 |
| ⚠️ 注册商不是客户 | 可能前服务商代持 | 必须先确认客户有管理权限 |

**出问题时**（第 4–6 步卡住）→ 先跑巡检定位，再照 §6 速查表逐行处理；证书长期卡住见 §5-c。
...
## 五-b、SSL 证书检测（本地先行 + 线上复核）

> 巡检内置**双层检测**，因为平台状态可能滞后或误报（实测：平台写 `failed`，本地实测证书正常且 HTTPS 200）。

**第一层｜本地实测**（`probe_tls`，stdlib socket+ssl，零依赖跨平台）：

| 检查 | 说明 |
|---|---|
| TLS 握手 | 443 是否可建立 TLS 连接 |
| 证书校验 | CA 可信 + **域名匹配** + 未过期（三合一，任一项失败即 `verified=False`） |
| 失败原因分类 | 区分「证书域名不匹配 / 已过期 / 自签名 / 证书链不完整 / 握手失败 / 连接超时」 |
| **服务端实际提供什么证书** | 校验失败时解析对端证书，回答"对方到底给了我哪张证书"（用 stdlib DER 解码取 SAN/CN，结果结构化、无噪声）——**尽力解析，仅供人工参考** |
| 到期天数 | 剩余 ≤30 天给提醒 |

**第二层｜线上可达性**（`probe_http`，curl 跟随跳转）：

- 证明"网站真的能打开"，而非只有握手成功
- 记录最终状态码、跳转次数、落点（可验证 301 是否跳对目标）
- **内置重试**：单次网络抖动不报错（实测遇过连测 5 次全过但偶发 1 次失败）
...
|---|---|---|
| ✅ | `active` | 全绿 |
| ✅ | `failed`/`none` | ⚠️ 平台状态滞后 → `refresh_domain` 复查（**不要**让客户去"申请证书"） |
| ❌ | `active` | ❌ **两侧矛盾** → 需排查（证书被吊销 / 域名改指了别处） |
| ❌ | `none`/`requested` | ⚠️ 平台正在签发 → 过渡态，等待即可 |

**实测案例**（2026-09-15）：`goods-suppliers.com` 平台 `certificateStatus=failed`，但本地 TLS 探测显示 **CN 正确、HTTPS 200** —— 平台状态滞后。巡检据此只给 ⚠️ 提醒而非 error。

**线上可达性的判定条件**（`probe_http`，缺一不可）：状态码 200 **且** 落点属同站（本域 / www.本域 / 站点运行时域名）**且** 内容不含 Next.js 错误壳标记 **且** 长度 > 1000 字节。只认 200 会漏掉「域名被改指到第三方站」和「首页变成运行时错误壳但 200」两类真实故障。
...
**删除后重新绑定的完整实测（2026-09-15，3 个域名一起重走）**：

```
删除 3 个域名（0/3）→ 绑前体检 → 重新绑定 3 个 → alias 全部 pending
[+  52s] alias active=2/3  网站 200
[+ 119s] alias active=3/3  网站 200   ← 约 2 分钟全部恢复
```

- **DNS 记录与证书都还在**的前提下（即刚删不久就重加），EdgeOne 重配**约 2 分钟**完成，`cname`/`ssl` 直接就是 `active`；
- 若证书已过期或被清（如 §5-c 的 failed 场景），则需重新走签发流程（约 3 分钟）；
- 中途 `alias=pending` 期间巡检会报 SSL 矛盾（平台 active / 本地证书不匹配）——**属预期中间态**，等 `alias=active` 后复查即可。
...
| **证书续期** | 平台自动处理，无需客户操作 |
| **根域与邮箱** | **Cloudflare**：根域 CNAME 被强制展平，走 §四 301 方案；**阿里云**：实测 @ 的 CNAME 与 MX/SPF 可共存（§3.3，2026-09-15，平台 cnameStatus=active），但属**服务商特有行为、非 DNS 标准保证**——邮箱重要的域名先确认收发正常再上线，拿不准就只配 www（§3.4） |
...
| **域名过期** | 绑前体检（`--check-domain`）会查注册剩余天数，≤30 天给续费提醒；**日常巡检不查到期**，绑定后的续费需人工留意 |
...
| 7a | 阿里云「添加记录」表单（CNAME / @） | ✅ 已嵌入 | §3.2 ① |
| 7b | 阿里云「解析变更确认」弹窗 | ✅ 已嵌入 | §3.2 ② |
| 7c | 阿里云两条记录添加成功（与 MX 共存） | ✅ 已嵌入 | §3.2 ③ |
