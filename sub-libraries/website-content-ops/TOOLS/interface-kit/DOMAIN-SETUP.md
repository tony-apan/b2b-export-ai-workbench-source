...
| **服务端实际提供什么证书** | 校验失败时解析对端证书，回答"对方到底给了我哪张证书"——实测价值极高：能一眼看出是云商默认证书（如 `*.cdn.myqcloud.com`）还是漏签发 |
| 到期天数 | 剩余 ≤30 天给提醒 |
...
**实测案例**（2026-09-15）：`goods-suppliers.com` 平台 `certificateStatus=failed` 且提示"未触发免费证书申请"，但本地 TLS 探测显示 **CN 正确、HTTPS 200** —— 平台状态滞后。巡检据此只给 ⚠️ 提醒而非 error。
