...
description: "把 English-only query contract、六槽位因果合同、阶段化正文/CTA/内链、可选 buyer roles、九段产品决策和证据门禁落实为可审查的 B2B SEO article draft。"
type: "template"
...
keywords: ["article draft", "English-only", "primary query", "direct answer", "six-slot causal contract", "product decision map", "progressive profiling", "stage CTA"]
record_type: "article-draft"
package_id: "replace-with-shared-package-id"
brief_id: "replace-with-approved-brief-id"
...
article_title: "replace-with-title-that-matches-the-completed-buyer-task"
page_h1: "replace-with-exact-page-shell-h1"
slug: "replace-with-human-readable-slug"
meta_description: "replace-with-bounded-task-and-outcome-description"
excerpt: "replace-with-scan-friendly-approved-excerpt"
...
in_scope_questions: ["replace-with-question-this-page-answers"]
...
direct_answer: "replace-with-materially-restated-bounded-answer"
direct_answer_placement_status: "missing"
...
primary_buyer_evidence_refs: ["replace-with-evidence-ref"]
...
cta_primary_path: "replace-with-stage-primary-action"
...
adapter_id: "replace-with-adapter-id"
persisted_format: "replace-with-persisted-format"
format_features: ["replace-with-verified-format-features"]
...
<!-- INTERNAL CONTROL RECORD: everything outside the unique publishable-body markers is workflow guidance and MUST NOT be sent to the CMS body. -->
# Article Draft
...
> Exact projection: `page_h1`、`terminal_action_contract`、`first_round_expected_output`、`candidate_decision_required_gates` 与 `first_round_output_candidate_gate_verdict` 必须和 Brief、Review、Publish Record 逐字符一致。`validate-technical` 必须保留 exact scalar `packet completeness, missing-evidence list, and next review step`；其他 branch 必须改为 exact scalar `not-applicable`。Validate 首轮不得出现 candidate-or-stop；只有完整第二轮 + named technical-owner review 后才可进入该后续决策。Fallback route request 必须与 terminal action 分离。
...
| `validate-technical` | 4–6 minimum engineering inputs; both arrays match exactly | Populate both arrays and one exact relationship row per second-round item | First round returns only completeness, missing evidence and next review step; candidate-or-stop requires complete second round plus named technical-owner review. |
...
- Target language: English (`en`) only.
- Target market:
- Primary query:
- Intent class / decision stage:
- Dominant buyer task:
- Query evidence status / refs:
- CTA interaction type and input-collection applicability / reason:
- Stage-link requirement status / reason:

```text
action | object | observable output | stage | commercial commitment (`none|soft|commercial`)
```

Stop and route to human language review if the intended article is not English. Market is an independent targeting field and must not be used to infer language.

## 2. Title, metadata and opening
...
### Opening answer contract

Keep the buyer-visible opening before the first H2 to about 120–140 English words as an editorial target, not a fatal word-count gate. It may differ from the front-matter wording, but it must naturally include: fictional/synthetic disclosure when applicable; the single buyer task; a narrow ICP fit statement; an explicit adjacent-audience or buying-condition exclusion; the first-round observable output; and the commitment boundary. The same opening must coherently implement all six direct-answer slots:

```text
action + object + required inputs/evidence + condition/boundary + expected output/route + evidence boundary
```

> Replace this block with the bounded direct answer. Before the first H2, let the target buyer self-identify and let an excluded buyer exit. State when the recommendation applies, when to stop, and what evidence remains necessary.

Do not open with company history, industry generalities or an unbounded benefit claim.

## 3. Six-slot Buyer Pain-to-Decision causal chain

| Node | Visible draft statement | Evidence ref | Boundary / uncertainty |
|---|---|---|---|
| Actor |  |  |  |
| Operating event |  |  |  |
| Evidence gap |  |  |  |
...
The six slots must form one causal chain, not six paraphrases. In the dedicated buyer-visible pain H2, render exactly six consecutive numbered natural-language lines that uniquely map, in order, to actor, trigger, evidence gap, rework, consequence and decision. Do not expose the internal audit labels `Actor:`, `Trigger:`, `Evidence gap:`, `Rework:`, `Consequence:` or `Decision:` in buyer-visible copy; those labels remain only in `visible_pain_chain`. Use bold sparingly for the decision, evidence gap, stop condition or next action rather than for schema labels. Do not invent customer quotations, downtime, ROI, loss rates, ranking or conversion effects. When pain evidence is inferred or synthetic, every buyer-visible pain, rework, delay, cost and consequence statement must use bounded `may/can/risks` language, never unverified `must/will`.

## 4. Decision path and hierarchy

Keep the CMS/page-shell title as the single page H1 outside the publishable body. Start the bounded article body at decision-led H2s; use H3 only to support the parent decision. Remove any section that does not help complete the dominant buyer task.

## [H2: State the first decision question or action]

- Applicable condition:
- Decision variable:
- Evidence to inspect:
- Boundary or no-fit condition:

### [H3: Supporting evidence or method]

Replace with a concrete method, table, checklist, calculation boundary or first-party artifact.

## [H2: Show how to reach the observable output]

Use ordered steps, a comparison table, a diagnostic sequence or a validation checklist appropriate to the selected stage.

## [H2: State stop conditions and remaining inputs]

Make uncertainty operational: state what is unknown, who must supply it and what next validation can resolve it.

## 5. Product decision map

| Condition | Variable | Evidence | No-fit | Remaining inputs | Candidate or stop | Candidate target | Next-validation target | Placement |
|---|---|---|---|---|---|---|---|---|
|  |  |  |  |  |  |  |  |  |

A non-stop candidate must link to a real product, solution or task-accepting hub. A stop row may use `N/A` for candidate target, but it still needs an executable next-validation target. Missing inputs belong under `remaining inputs` and route to `needs-follow-up`; no-fit is reserved for an evidenced incompatibility, out-of-envelope result or unsupported scope.
...
Add only roles supported by evidence:

```text
role | evidence_ref | concrete objection | article-owned answer
```

Engineer, Quality, Procurement and Management are reviewer lenses, not mandatory article sections.

### Cross-role delegation

If the article hands a task to another role or page, bind the full contract:

```text
from_role | to_role | url | retained_task | receiving_task | owner | acceptance_evidence_ref
```

## 7. Information gain and evidence placement

- First-party artifact shown:
- Artifact evidence status:
- Market-difference evidence status:
- SERP gap addressed:
- Limitation:

Synthetic fixtures may prove the artifact structure only. They cannot confirm a market difference, production capability or ranking advantage.

## 8. Semantic emphasis for scanning

Use bold only for a complete decision judgment, not for keyword decoration or whole paragraphs.
...
- **Condition:** replace with a complete applicability judgment.
- **Evidence:** replace with the exact evidence required before proceeding.
- **No-fit:** replace with a clear stop or alternate route.
- **Action:** replace with the next bounded action.

## 9. Stage-specific internal links

Add only the link roles needed by the selected stage:

| Stage | Default required link role | Optional link boundary |
|---|---|---|
| `learn` | `educational|hub` | Use only the role that accepts the next learning task |
| `troubleshoot` | `diagnostic|support` | Only post-diagnosis `technical-review|product` |
| `compare` | `comparison` | Evidenced candidates may add `product|solution|hub` |
| `validate` | `technical-review` | Candidate non-stop may add `product|solution|hub` |
| `buy` | `product|solution` + `conversion|commercial` | Educational support is not a substitute for the required buying path |
...
Do not add a universal product + education + conversion quota. Cross-role links also require the delegation contract in section 6.

## 10. Stage-specific CTA

Render only the CTA interaction selected by `stage_intake_contract`. The buyer-visible CTA must state what to prepare, what the buyer receives, how to submit, who responds, whether timing is verified or unknown, what not to submit, and what the action does not commit the buyer to.

- `none`: no form, no hidden intake and no human-response promise; offer the declared self-check, local tool or next reading.
- `troubleshoot-support`: request one minimal diagnostic packet and return a support-ready route; no second round and no technical qualification language.
- `compare-handoff`: request only the evidenced comparison-handoff fields and return the bounded comparison next step; no Validate lifecycle.
- `validate-technical`: request the 4–6 canonical first-round inputs and return only packet completeness, missing-evidence list and next review step. Request the canonical second round only as the next bounded review step; no candidate-or-stop conclusion is permitted until that second-round universe is complete and a named technical owner has reviewed it.
- `buy-commercial`: request the independent commercial packet and route it to the named commercial owner; do not reuse technical-qualified as commercial acceptance.

The preparation path, copyable-message fallback and review/request path must use the same selected-stage packet in the same order. Do not expose internal labels such as `Soft CTA`, `Final CTA`, lifecycle states or schema field names inside the publishable markers.

Only `validate-technical` may describe the second-round relationship and named technical follow-up owner. Every other branch keeps second-round and `technical_qualification_*` fields empty/not-applicable.

Use this buyer-visible skeleton for every applicable CTA. The complete input set, units and examples must exist only once in the local worksheet/fallback artifact; the CTA references that worksheet and must not relist the packet:

```markdown
Trigger: [state the exact condition that makes the next action appropriate]

Preparation: [complete the single worksheet defined earlier; do not repeat its field list]

Expected output: [state the observable buyer-facing return]

Who responds: [named owner or team]

Data boundary: [state what not to include and link to the buyer-facing data-handling notice]

Validation boundary: [state what the action and output do not prove]

Route availability: [verified route with evidence, or unverified-unavailable / endpoint not-applicable]

> [insert the exact cta_fallback_message_template when the primary route is unavailable; refer to the worksheet instead of restating all fields]
```

For `unverified-unavailable` or any route/data-policy/retention/deletion/owner-acceptance BLOCK, the visible fallback must say: do not submit, send, email, upload, share or transmit; save the packet locally; use the buyer organization's existing approved supplier-contact process to request verified route and policy details. Do not place a reserved or synthetic endpoint in the CTA body. This fallback request is not the `terminal_action_contract`.

## 11. Qualification boundary

Use only `qualification_reason_codes` rows legal for the selected stage/intake:

| Branch | Allowed control-record scope |
|---|---|
| `none` | No qualification rows. |
| `troubleshoot-support` | Missing diagnostic input or evidenced support stop/redirect only. |
| `compare-handoff` | No technical lifecycle; a commercial bridge remains a bounded handoff, not acceptance. |
| `validate-technical` | First-round completeness, named technical review, second-round completion, evidenced technical no-fit and bounded technical qualification. |
| `buy-commercial` | Explicit commercial qualification, evidenced commercial stop/redirect and named commercial-owner acceptance only. |

Control-record states never appear in buyer-visible body. Missing evidence is not an evidenced no-fit; technical readiness is not sales acceptance.

## 12. Evidence gates

For each applicable CTA and internal-link axis, record all three states independently:

| Object / axis | Check execution status | Evidence result | Gate verdict | Evidence refs |
|---|---|---|---|---|
...
A production-applicable PASS requires `executed + confirmed + pass`. Missing or not-run evidence cannot be PASS. Synthetic fixtures may pass structure while production evidence remains `not-applicable` or BLOCK.

## 13. Close and next task

Restate the bounded decision, stop condition and next stage-appropriate action. `terminal_action_contract` must match the final buyer-visible local action; fallback route recovery remains separate. `first_round_expected_output`, `candidate_decision_required_gates`, and `first_round_output_candidate_gate_verdict` must exactly match the other three records, and a Validate first round must contain no candidate-or-stop. Do not promise rankings, inquiries, sales acceptance, revenue or conversion lift.


## 14. Publishable body handoff

Only the final buyer-facing article body belongs between the markers below. Keep the CMS title outside the body, so the bounded body contains no H1. Before handoff, remove every placeholder and internal-control term from the bounded body.

<!-- PUBLISHABLE_BODY_START -->

<!-- PUBLISHABLE_BODY_END -->

The bounded body must not contain `# Article Draft`, schema-parity notes, evidence-gate labels, renderer/release status, `replace-with-*`, bracketed heading placeholders, or an H1. Marker absence, duplication, reversal or content conversion outside this unique range is BLOCK.

## 15. Renderer and frontend boundary
...
An image alt value in Markdown, Slate or media metadata does not prove that the final DOM rendered a non-empty alt attribute.
