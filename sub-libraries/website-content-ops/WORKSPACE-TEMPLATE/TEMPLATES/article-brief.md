...
description: "把 English-only query contract、六槽位因果合同、阶段化 CTA/内链、可选 buyer roles、cannibalization、产品决策和证据门禁转成可审查的 B2B SEO article brief。"
type: "template"
...
related: ["article-draft.md", "article-quality-review.md", "publish-record.md", "../30_tasks/article-page-frontend-seo-contract.runtime.md"]
visibility: "public"
redaction_status: "safe-to-publish"
when_to_read: "需要把英文买家问题、搜索意图、产品事实、站内内容职责和阶段化 CTA 合同整理为可写作、可评分、可发布验收的正式 B2B SEO brief 时。"
keywords: ["article brief", "English-only", "primary query", "dominant buyer task", "six-slot causal contract", "stage CTA", "technical qualification", "sales acceptance"]
...
production_snapshot_freshness_verdict: "block"
production_snapshot_provenance_verdict: "block"
icp_evidence_status: "replace-with-confirmed-inferred-missing-conflicting-or-expired"
icp_evidence_refs: []
icp_fit_contract: "replace-with-company-fit-application-context-buying-condition-and-required-capability-to-act"
icp_exclusion_contract: "replace-with-adjacent-audiences-applications-or-buying-conditions-this-article-must-not-attract"
fixture_contract_status: "missing"
supported_content_languages: ["en"]
target_content_language: "en"
target_market: "replace-with-market"
language_gate_verdict: "block"
working_article_title: "replace-with-title-that-matches-the-completed-buyer-task"
page_h1: "replace-with-exact-page-shell-h1"
primary_query: "replace-with-one-real-search-trigger"
supporting_query_variants: ["replace-with-same-task-close-variant"]
...
excluded_query_modifiers: ["replace-with-competing-modifier-owned-elsewhere"]
...
expected_content_type: "replace-with-serp-aligned-content-type"
...
stage_primary_outcome: "replace-with-stage-specific-reader-completion"
stage_cta_mode: "replace-with-self-check-education-diagnostic-support-comparison-bounded-engineering-review-rfq-or-commercial-qualification"
stage_required_link_roles: ["replace-with-one-or-more-of-hub-product-solution-educational-comparison-diagnostic-support-technical-review-conversion-commercial"]
stage_sales_qualification_requirement: "replace-with-not-applicable-optional-or-required"
...
buyer_task_evidence_refs: ["replace-with-source-path-or-record-id"]
search_demand_evidence_status: "missing"
search_demand_evidence_refs: ["replace-with-search-evidence"]
...
serp_format_evidence_refs: ["replace-with-dated-serp-evidence"]
production_search_evidence_gate_verdict: "block"
serp_gap: "replace-with-evidence-bound-gap-or-mark-as-hypothesis"
serp_gap_status: "missing"
serp_gap_refs: ["replace-with-dated-serp-evidence-or-synthetic-boundary-card"]
...
pain_trigger: "replace-with-specific-trigger-event"
surface_problem: "replace-with-visible-problem"
operational_friction: "replace-with-workflow-or-confidence-friction"
business_consequence: "replace-with-bounded-business-consequence"
desired_decision: "replace-with-decision-the-reader-needs-to-make"
primary_buyer_role: "replace-with-primary-role"
primary_buyer_objection: "replace-with-primary-role-concrete-objection"
primary_buyer_answer: "replace-with-article-owned-answer"
secondary_buyer_roles: []
secondary_buyer_role_contracts: []
reviewer_lenses_not_served_as_buyer_roles: []
role_handoff_contracts: []
content_inventory_status: "missing"
content_inventory_refs: ["replace-with-content-inventory"]
cannibalization_status: "unresolved"
owner_page: "replace-with-canonical-owner-page"
conflict_candidates: []
intent_separation: []
inventory_zero_result_evidence_refs: []
inventory_snapshot_ref: "replace-with-inventory-snapshot-record"
inventory_checked_at: "replace-with-iso-date"
content_action: "replace-with-create-update-merge-redirect-or-do-not-write"
first_party_proof_status: "missing"
first_party_proof_refs: ["replace-with-proof-path-or-record-id"]
information_gain_artifact_status: "missing"
information_gain_artifact_refs: ["replace-with-artifact-or-section-ref"]
market_information_gain_status: "missing"
information_gain_market_refs: ["replace-with-dated-market-difference-evidence"]
product_decision_map_status: "missing"
product_link_evidence_level: "replace-with-none-family-level-or-sku-level"
product_decision_map: ["replace-with-condition|replace-with-variable|replace-with-evidence|replace-with-no-fit|replace-with-remaining-inputs|replace-with-candidate-or-stop|replace-with-candidate-target|replace-with-next-validation-target|replace-with-placement"]
product_claim_ledger_applicability: "replace-with-applicable-or-not-applicable"
product_claim_ledger_not_applicable_reason: "replace-with-reason-or-not-applicable"
product_claim_ledger: []
product_claim_ledger_verdict: "block"
component_intent_status: "missing"
semantic_emphasis_plan: ["replace-with-condition-risk-evidence-no-fit-boundary-action-or-decision|replace-with-complete-judgment|replace-with-placement"]
...
internal_link_anchor_task_parity_verdict: "block"
...
cta_contract_status: "missing"
cta_trigger: "replace-with-stage-qualified-trigger"
cta_required_inputs: []
cta_buyer_visible_capability_proofs: []
cta_input_collection_mode: "progressive-profiling"
cta_input_alignment_status: "missing"
cta_progressive_profiling_status: "not-applicable"
cta_progressive_profiling_omitted_inputs: []
cta_progressive_profiling_followup_action: "not-applicable"
cta_progressive_profiling_followup_owner: "not-applicable"
cta_complete_over_six_justification: "not-applicable"
cta_soft_path: "replace-with-self-check-worksheet-or-input-preparation-path"
cta_fallback_message_template: "replace-with-copyable-message"
...
cta_expected_output: "replace-with-stage-appropriate-deliverable"
cta_validation_boundary: "replace-with-what-the-output-does-not-prove"
cta_destination: "replace-with-real-stage-appropriate-destination"
cta_owner: "replace-with-accountable-output-owner"
...
generated_from: "../../TEMPLATES/article-brief.md"
generated_source_sha256: "85b7d2565c928f58c411d57ab8630c3163ccd0ddc404bb6b2ba9432098b7d870"
generated_by: "scripts/sync-workspace-template.mjs"
---
<!-- Generated runtime copy from TEMPLATES/article-brief.md; do not hand-edit this copy in the source package. -->
# Article Brief

> Canonical projection contract: `content_action`、`page_h1`、`dominant_search_intent`、`terminal_action_contract`、`first_round_expected_output`、`candidate_decision_required_gates`、`first_round_output_candidate_gate_verdict`、`content_family_matches`、`content_family_singleton_verdict`、customer-language/pain status+refs+gate、`evidence_origin`、`fixture_identity`、`production_proof_eligible`、`cta_transmission_action_inventory`、`cta_collection_route_policy_contracts`、`section_information_gain_verdict` 与 `normalized_field_set_redundancy_verdict` 必须与其余三记录逐字符一致，不得使用 snapshot alias 或省略。Production 只允许 `live-production + fixture_identity=not-applicable + production_proof_eligible=true`，并要求 customer language 与 pain evidence 都是 `confirmed + 非空 refs + gate pass`；synthetic/test fixture 必须 `production_proof_eligible=false`，不能成为 production proof。
...
| `validate-technical` | 4–6 minimum engineering inputs; both arrays match exactly | Populate both arrays and one exact relationship row per second-round item | First round returns only packet completeness, missing evidence, and next review step. Candidate-or-stop requires the complete second-round package plus named technical-owner review. |
...
`qualification_reason_codes` contains only states legal for the selected branch. Never copy the full Validate lifecycle into Learn, Troubleshoot, Compare, or Buy. For `validate-technical`, keep `first_round_expected_output` byte-for-byte as `packet completeness, missing-evidence list, and next review step` and set `candidate_decision_required_gates` to complete-second-round-package + named-technical-owner-review. For every non-Validate branch, replace `first_round_expected_output` with the exact scalar `not-applicable` and set both gate items to `not-applicable`. A first-round candidate decision is a fatal BLOCK.
...
## 1. Language and market gate

- Supported languages: English only (`en`).
- Target content language: `en`.
- Target market:
- If the source or intended output is not English: stop and route to human language review.
- Do not treat target market as a language field.

## 2. Query and dominant buyer task

### Query contract

- Query evidence status / refs: record whether the primary query, same-task variants, and excluded modifiers are `confirmed`, `inferred`, or `missing`. `synthetic-only` is reserved for CTA/internal-link evidence results and is not a query fact status.

- Primary query:
- Same-task supporting variants:
- Excluded modifiers and owner pages:
- Intent class:
- Stage:
- Expected SERP content type:
- SERP evidence ref:

Each supporting variant must complete the same action, object and observable output as the primary query. Remove price/MOQ, definition, troubleshooting, validation or RFQ modifiers when they introduce a different stage or commercial commitment.

### Dominant task grammar
...
- Observable output:
- Stage:
- Commercial commitment (`none|soft|commercial`):
- Technical CTA/action/output (for example `bounded-engineering-review`) is recorded separately and must not be encoded as a commercial commitment.
- Completion test:
- In scope:
- Out of scope:

## 3. Stage and intake contract

| Stage | Canonical `stage_intake_contract` | Primary outcome | Allowed CTA | Required link roles | Intake boundary |
|---|---|---|---|---|---|
| `learn` | `none` | Self-check or understanding | Self-check / education | `educational|hub` | No form by default; never inherit technical or commercial qualification |
| `troubleshoot` | `none` by default; `troubleshoot-support` only with a real support destination and accepted receiving task | Diagnosis/support readiness | Diagnosis / support | `diagnostic|support`; only post-diagnosis `technical-review|product` | Keep `none` for in-page diagnosis. Upgrade only for a real support handoff, then collect only current diagnostic inputs. |
| `compare` | `none` by default; `compare-handoff` only with dated `mixed-commercial` evidence, a real destination, named owner and accepted receiving task | Comparable dimensions or shortlist | Comparison | `comparison`; evidenced candidates may add `product|solution|hub` | Keep `none` for self-contained comparison. Upgrade only for the separately evidenced mixed-commercial packet. |
| `validate` | `validate-technical` | Bounded validation decision | Engineering review | `technical-review`; candidate non-stop may add `product|solution|hub` | Technical progressive refinement may apply; sales acceptance remains separate |
| `buy` | `buy-commercial` | RFQ/commercial next step | RFQ / commercial qualification | `product|solution` + `conversion|commercial` | Use an RFQ/commercial packet; do not inherit the Validate technical lifecycle |

Selected stage, intake contract and rationale:

## 4. Six-slot Buyer Pain-to-Decision causal chain

| Node | Evidence-backed statement | Evidence ref | Uncertainty/boundary |
|---|---|---|---|
| Actor |  |  |  |
| Operating event |  |  |  |
| Evidence gap |  |  |  |
...
Do not fabricate customer wording, loss figures, ROI, downtime or conversion impact.

## 5. Direct answer contract

The opening may materially restate rather than exactly copy the front matter, and it must not force a visible `Direct answer:` label. Complete all six slots coherently:

```text
action | object | required inputs or evidence | condition or boundary | expected output or route | evidence boundary
```
...
- Condition/boundary:
- Expected output or route:
- Evidence boundary, including what is not proven:
- Placement: before the first H2, keep the complete opening answer at about 120–140 English words; include fictional/synthetic disclosure when applicable, buyer task, narrow ICP fit, exclusion, self-identification, first-round output, commitment boundary, and all six machine-contract slots without requiring visible schema labels.

## 6. Buyer-role scope and delegation
...
### Evidence-based secondary buyers
...
Engineer, Quality, Procurement and Management remain reviewer lenses; they are not mandatory article sections.

## 7. Inventory and cannibalization

- Owner page:
- Snapshot ref / checked at:
- Status: clear / resolved / unresolved
- Action: create / update / merge / redirect / do-not-write

For `clear`, zero candidates are valid only with zero-result evidence. For `resolved`, use matching rows:
...
## 8. Evidence and information gain

- Buyer-task evidence:
- Search-demand evidence:
- SERP-format evidence:
- SERP gap and boundary:
- First-party proof and limitation:
- Information-gain artifact:
- Artifact evidence status:
- Market-difference evidence status:

Synthetic fixtures may demonstrate artifact shape but cannot claim confirmed market difference.

### Conditional FAQ decision

FAQ is optional. Mark it `applicable` only when a dated, real SERP/query pattern for the same dominant buyer task or a specific buyer objection/uncertainty creates an article-owned question that the main decision path does not already answer naturally. Do not add FAQ merely because it is described as an SEO best practice.

- FAQ applicability (`applicable|not-applicable`):
- Trigger type (`serp-query-pattern|buyer-objection|not-applicable`):
- Trigger evidence refs (use `[]` when not applicable):
- Absence reason (required when not applicable):

When applicable, add one row per question:

```text
question | buyer_job | objection_or_uncertainty | evidence_ref_or_explicit_inferred_boundary | article_owned_answer
```

The buyer job must match the dominant task or an evidenced secondary buyer task. If a row is inferred, label it `inferred`, state the basis and the non-generalization boundary; do not present it as confirmed production evidence. Remove questions that repeat the body, stack synonymous keyword variants, create another stage, or belong to a different owner page. With no eligible trigger evidence, FAQ absence is valid and must not be penalized.

## 9. Product decision map

| Condition | Variable | Evidence | No-fit | Remaining inputs | Candidate or stop | Candidate target | Next-validation target | Placement |
|---|---|---|---|---|---|---|---|---|
|  |  |  |  |  |  |  |  |  |

A non-stop candidate requires a real product, solution or task-accepting hub URL. A stop row may use `N/A` for candidate target but still requires an executable next-validation target. Missing inputs belong in `remaining inputs` and route to `needs-follow-up`; they are not a no-fit condition. Use no-fit only for an evidenced incompatibility, out-of-envelope result or unsupported scope.

## 10. Hierarchy and semantic emphasis

Reserve a buyer-visible opening before the first H2 with about 120–140 English words as an editorial target, not a fatal word-count gate. It must include applicable fictional/synthetic disclosure, dominant task, narrow ICP fit, explicit exclusion, first-round output and commitment boundary so the intended buyer can self-identify immediately. The dedicated buyer-visible pain H2 must use exactly six consecutive numbered natural-language lines that uniquely map, in order, to actor, trigger, evidence gap, rework, consequence and decision. Keep the literal audit labels only in `visible_pain_chain`; do not expose `Actor:`, `Trigger:` or the other internal labels in buyer-visible copy.
...
Semantic strong grammar:

```text
decision-scanning-role | complete judgment | placement
```

Allowed roles: `condition / risk / evidence / no-fit / boundary / action / decision`.

## 11. Stage-specific internal links

Add only roles required by the selected stage. Do not satisfy a universal link quota.
...
Cross-role links also require the seven-part delegation contract.

## 12. CTA and qualification

Apply only the branch selected by `stage_intake_contract`:

- `none`: keep `first_round_inquiry_inputs`, `second_round_inquiry_inputs`, `second_round_input_relationships`, `cta_required_inputs`, `technical_qualification_gates`, and `qualification_reason_codes` empty; provide only the declared self-check/local action.
- `troubleshoot-support`: put the minimal diagnostic packet in `first_round_inquiry_inputs` and copy it exactly to `cta_required_inputs`; keep every second-round and technical-qualification field empty/not-applicable.
- `compare-handoff`: put only the evidenced comparison-handoff packet in the first-round/CTA arrays; keep the second round and technical lifecycle empty.
- `validate-technical`: use 4–6 minimum first-round inputs, an exact CTA copy, the bounded second-round universe, one relationship row per second-round item, and the technical qualification fields. The first-round output is limited to packet completeness, missing evidence and next review step; candidate-or-stop is allowed only after the complete second round and named technical-owner review.
- `buy-commercial`: put the independent RFQ/commercial packet in the first-round/CTA arrays; keep second-round and technical qualification fields empty, and use only commercial owner/state rows.

For any applicable CTA, record trigger, buyer-visible output, response expectation, submission method, data boundary, commitment boundary, destination, buyer-visible owner, preparation path and copyable fallback. These visible paths must request the same selected-stage packet in the same order. `terminal_action_contract` describes only the local page-completable action; requesting a verified route is an independent fallback. If any route or policy gate is BLOCK, instruct the buyer only to keep the packet local and request verified route/policy details—never to submit, send, email, upload, share or transmit it.

Only `validate-technical` uses this relationship grammar:

```text
second_round_item|new-or-refines|first_round_item-or-not-applicable|additional_decision_purpose
```

`new` uses `not-applicable` in the third slot; `refines` names the exact first-round summary it deepens. Do not request the same value twice.

Populate `qualification_reason_codes` with `state|cause-category|evidence-rule|owner|next-step` rows only for states legal in the selected branch. Missing evidence is follow-up, not automatic disqualification; technical readiness never means sales acceptance.

## 13. Three-axis evidence gates

For CTA and internal links, record each applicable axis independently:

| Axis | Check execution | Evidence result | Gate verdict | Evidence refs |
|---|---|---|---|---|
| Reference parity |  |  |  |  |
| Reachability |  |  |  |  |
| Capability/task acceptance |  |  |  |  |

Synthetic structure PASS must keep production evidence `not-applicable` or gate BLOCK. Missing/not-run cannot be PASS.

## 13A. Semantic and evidence closure

- `content_family_matches` 必须由全部 supported family patterns 重算，且恰好一个；0 或多个 match 均 BLOCK。
- `terminal_action_contract` 与 dominant task、Act row、stage 和 commitment 必须逐字符一致；fallback route request 必须分离。`first_round_expected_output`、`candidate_decision_required_gates` 与 `first_round_output_candidate_gate_verdict` 四记录 exact projection；Validate 首轮不得出现 candidate-or-stop。Learn/Troubleshoot/Compare/Validate 不得以 supplier nomination/appointment/award、quote、RFQ 或 order 为终点。
- `visible_pain_chain` 必须在 Brief、Draft、Review、Publish Record 四记录逐字符 exact projection，并保留 `Actor|...` → `Trigger|...` → `Evidence gap|...` → `Rework|...` → `Consequence|...` → `Decision|...` 的机器控制标签。正文在一个专用 H2 下使用六条连续编号自然语言逐项绑定这六槽，但不得公开内部 audit labels；缺编号、缺槽、重复、调序、相邻因果不成立或无法唯一映射均 fatal BLOCK。可用 `**...**` 强调真正的痛点、判断或 stop condition，不得用加粗伪装 schema。`pain_evidence_status=inferred` 或 synthetic fixture 的 buyer-visible pain 只能写 `may/can/risks`，不得写无证据的 `must/will`。
- 产品链接 anchor 必须由 anchor 本身表达 buyer task、decision、evidence gap 或 expected output；软广 anchor 不合格。
- `cta_transmission_action_inventory` 一行一个传递动作：`surface-id|normalized-action|object|instruction-mode|route-id-or-not-applicable|route-status|evidence-bundle-ref-or-not-applicable`。未验证 route 只能 local-only、prohibited-until-verified 或 conditional-after-verification。
- 适用产品事实必须进入 `product_claim_ledger`：`claim-id|buyer-visible-claim|claim-type|evidence-ref|applicability-boundary|target-url-or-not-applicable|target-page-parity-status|production-use-status`。Synthetic card 不是 Production SKU proof。
- stable IDs 只用于 identity join；Production reviewer 必须与 author、producer、remediation participants 分离并有 provenance。Fixture/AI self-review 不能给 Production independence PASS。
- Production snapshot common envelope 必须含 `schema_version, artifact_kind, evidence_scope, captured_at, subject_id, scope_id, capture_method, producer_id, independent_reviewer_id, payload`；四种 artifact kind 的 payload 非空、closed-schema、默认不超过 395 天，并由 digest 绑定真实 bytes。
- Fixture、placeholder、`example.test` 与测试 digest 只证明 shape/validator branch。禁止排名、流量、询盘、转化、收入和 customer-acquisition 结果保证；实际结果保持 `unverified`。

## 14. Explicit non-claims and deferred blocks

This brief does not promise ranking, inquiries, sales acceptance or conversion lift.
