---
title: "Publish Record Template"
description: "记录 English-only B2B SEO 文章从已批准合同到 CMS mutation、三层回读、阶段化 CTA/内链、资格状态和生产证据门禁的可追溯发布结果。"
type: "template"
...
sources: ["../PLAYBOOKS/id-0001-b2b-seo-article-standard.md", "../PLAYBOOKS/id-0003-b2b-article-optimization-sop.md", "../ADAPTERS/cms/allincms/article-operations.md"]
...
keywords: ["publish record", "CMS readback", "production evidence", "stage CTA", "technical qualification", "sales acceptance", "frontend block", "outcome boundary"]
record_type: "publish-record"
package_id: "replace-with-shared-package-id"
brief_id: "replace-with-approved-brief-id"
draft_id: "replace-with-approved-draft-id"
review_id: "replace-with-passing-review-id"
publish_record_id: "replace-with-publish-record-id"
...
fixture_contract_status: "not-applicable"
supported_content_languages: ["en"]
target_content_language: "en"
target_market: "replace-with-market"
language_gate_verdict: "block"
primary_query: "replace-with-approved-primary-query"
supporting_query_variants: ["replace-with-approved-same-task-variant"]
...
excluded_query_modifiers: ["replace-with-approved-excluded-modifier"]
...
product_decision_map_snapshot: ["replace-with-condition|variable|evidence|no-fit|remaining-inputs|candidate-or-stop|candidate-target|next-validation-target|placement"]
...
primary_buyer_role: "replace-with-approved-primary-role"
...
owner_page: "replace-with-approved-owner-page"
inventory_snapshot_ref: "replace-with-dated-inventory-snapshot"
...
stage_cta_mode: "replace-with-learn-troubleshoot-compare-validate-or-buy-cta-mode"
stage_required_link_roles: ["replace-with-learn-troubleshoot-compare-validate-or-buy-link-role"]
...
visible_pain_chain: ["Actor|replace-with-exact-visible-row", "Trigger|replace-with-exact-visible-row", "Evidence gap|replace-with-exact-visible-row", "Rework|replace-with-exact-visible-row", "Consequence|replace-with-exact-visible-row", "Decision|replace-with-exact-visible-row"]
...
published_article_title: "replace-with-exact-draft-article-title"
page_h1: "replace-with-exact-page-shell-h1"
published_slug: "replace-with-exact-draft-slug"
published_meta_description: "replace-with-exact-draft-meta-description"
published_excerpt: "replace-with-exact-approved-excerpt"
...
cta_trigger: "replace-with-approved-trigger"
cta_required_inputs_snapshot: []
cta_buyer_visible_capability_proofs_snapshot: []
cta_input_collection_mode: "progressive-profiling"
cta_primary_path: "replace-with-approved-primary-path"
cta_soft_path: "replace-with-approved-soft-path"
cta_fallback_message_template: "replace-with-approved-copyable-message"
...
cta_expected_output: "replace-with-approved-output"
cta_validation_boundary: "replace-with-approved-boundary"
cta_destination: "replace-with-real-destination"
...
indexing_intent: "replace-with-indexing-intent"
publication_status: "not-published"
publication_lifecycle_evidence_rows: ["authorization|status|artifact-ref|sha256|site-id|record-id|url-or-not-applicable|producer-id|independent-reviewer-id|observed-at|reviewed-at|review-ceiling", "cms-mutation|status|artifact-ref|sha256|site-id|record-id|url-or-not-applicable|producer-id|independent-reviewer-id|observed-at|reviewed-at|review-ceiling", "backend-readback|status|artifact-ref|sha256|site-id|record-id|url-or-not-applicable|producer-id|independent-reviewer-id|observed-at|reviewed-at|review-ceiling", "editor-reopen|status|artifact-ref|sha256|site-id|record-id|url-or-not-applicable|producer-id|independent-reviewer-id|observed-at|reviewed-at|review-ceiling", "anonymous-frontend|status|artifact-ref|sha256|site-id|record-id|url|producer-id|independent-reviewer-id|observed-at|reviewed-at|review-ceiling", "desktop|status|artifact-ref|sha256|site-id|record-id|url|producer-id|independent-reviewer-id|observed-at|reviewed-at|review-ceiling", "mobile|status|artifact-ref|sha256|site-id|record-id|url|producer-id|independent-reviewer-id|observed-at|reviewed-at|review-ceiling", "image-fetch-decode|status|artifact-ref|sha256|site-id|record-id|image-url|producer-id|independent-reviewer-id|observed-at|reviewed-at|review-ceiling"]
...
## 1. Package, scope and authorization

- Task ID:
- Package / Brief / Draft / Review IDs:
- Article standard version / doc ID:
- CMS / exact site:
- Adapter version / profile:
- Content type:
- Mode: dry-run / draft / publish / update
- Target count:
- Indexing intent: index / noindex
- Human authorization context / exact boundary:
- Started at:
- Completed at:

This record is for English content only. `target_market` is independent from `target_content_language`. If the actual body is not English, record `language_gate_verdict: block` and stop for human language review.

## 2. Approved search and stage contract

Record query-evidence status/refs, CTA interaction type, input-collection applicability/reason, and stage-link requirement status/reason exactly as approved.
...
Exact four-record projection (copy byte-for-byte from the approved Brief, Draft and Review):
...
- Stage CTA mode:
- Required link roles:
- Sales qualification requirement:

The published page must match the approved contract. A successful CMS mutation does not authorize changing Learn into Buy, adding a sales CTA to every article or adding a universal three-link bundle.

## 3. Release gate snapshot

| Gate | Expected | Actual | pass/block/not-applicable | Evidence ref |
|---|---|---|---|---|
| Language | English `en`; market separate |  |  |  |
| Fatal gate review | PASS before approved publish |  |  |  |
| Structure review | Approved structure score and verdict |  |  |  |
| Production evidence | Applicable production evidence independently passes |  |  |  |
| Query/task/stage parity | Published fields and body retain approved task |  |  |  |
| Direct answer | Materially restates action + object + condition/boundary |  |  |  |
| Six-slot causal chain | Visible, specific and evidence-bounded |  |  |  |
| Product decision map | Nine parts visible; missing inputs route to follow-up; no-fit is evidence-based; non-stop candidate target is real |  |  |  |
| Buyer role scope | One primary buyer; secondary roles only when evidenced |  |  |  |
| Cannibalization | Dated inventory supports clear/resolved state |  |  |  |
| Information gain | Artifact status and market status remain separate |  |  |  |
| Unsupported outcomes | No ranking, inquiry or conversion promise |  |  |  |

Fatal gate BLOCK takes precedence over scores. Synthetic structure evidence cannot satisfy a production publish gate, and a synthetic package must record `production_evidence_score: not-applicable` rather than a numeric score.

## 4. Buyer, inventory and evidence snapshot
...
- Cannibalization status: clear / resolved / unresolved
- Zero-result evidence refs when clear with 0 candidates:
- Candidate/separation one-to-one evidence when resolved:
- Information-gain artifact status / refs:
- Information-gain market status / refs:
- Pain evidence origin: confirmed / inferred / synthetic
- Inferred/synthetic wording check: only `may` / `can` / `risks`; unsupported `must` / `will` absent

`clear` may have zero conflict candidates only when the dated inventory snapshot and zero-result evidence are recorded. `resolved` requires each candidate to have one matching separation row. Artifact structure, including a synthetic fixture, does not prove a market difference.

## 5. Target CMS fields

- SEO title: copy `published_article_title` exactly
- Page-shell H1: copy `page_h1` exactly
- Meta description: copy `published_meta_description` exactly
- Excerpt: copy `published_excerpt` exactly
- Slug: copy `published_slug` exactly
- Cover media ID / URL:
- Category IDs:
- Tag IDs:
- Author / reviewer:
...
- Final public URL:
- Persisted body format:
- Required format features:
- Content digest before mutation:
- Content digest after readback:

If this record renders a `Proposed search fields` table, its SEO title, meta description, H1 and excerpt cells must be byte-for-byte identical to the four frontmatter fields above; the table is a projection, never a second source of truth.

## 6. Stage-specific links and CTA snapshot
...
- Approved required stage role present:
- Optional links and rationale:
- Product/solution candidate binding:
- Cross-role delegation, if any:
...
Snapshot the selected stage branch exactly; do not reinterpret an approved Review:

- `none`: all intake and qualification arrays are empty; record the local completion action only.
- `troubleshoot-support`: `first_round_inquiry_inputs` and `cta_required_inputs_snapshot` contain the same minimal diagnostic packet; second-round and technical qualification fields stay empty/not-applicable.
- `compare-handoff`: snapshot only the approved minimal handoff packet; no technical lifecycle.
- `validate-technical`: snapshot the 4–6 first-round inputs, exact CTA copy, complete second-round inputs/relationships, named technical owner and bounded output.
- `buy-commercial`: snapshot the independent commercial packet and named commercial owner; second-round and technical qualification fields stay empty/not-applicable.

Also snapshot the buyer-visible value exchange, timing boundary, submission method, confidentiality/data boundary, commitment boundary, destination, owner, preparation path and copyable fallback. Keep `terminal_action_contract` separate from the fallback request for a verified route. If route or collection-policy status is BLOCK, do not expose a direct endpoint and do not instruct submit, send, email, upload, share, transmit or an equivalent delivery action; allow only local save plus a request for a verified route through the approved supplier-contact process. Do not use internal authoring labels in buyer-visible copy.

## 7. Qualification snapshot

Persist only the approved `qualification_reason_codes` rows for the selected branch, using `state|cause-category|evidence-rule|owner|next-step`. Do not copy a universal lifecycle table. `none` has no rows; Troubleshoot has only diagnostic follow-up/stop routing; Compare has no technical lifecycle; Validate may carry bounded technical states; Buy carries independent commercial states. The snapshot must match Review exactly and must not derive acceptance from submission alone.

## 8. Three-axis production evidence
...
A production-applicable PASS requires `executed + confirmed + pass`. Missing, not-run, a toast, HTTP 200 alone or a reserved URL must remain BLOCK. A synthetic fixture may pass structure while production evidence remains `not-applicable` or BLOCK.

## 8A. Semantic projection record

Publish Record 必须从已批准 Brief/Draft/Review 原样投影并重新验收：`page_h1`、`terminal_action_contract`、`first_round_expected_output`、`candidate_decision_required_gates`、`first_round_output_candidate_gate_verdict` 五字段逐字符一致；`validate-technical` 的 `first_round_expected_output` 只接受 exact scalar `packet completeness, missing-evidence list, and next review step`，其他 branch 只接受 `not-applicable`；exact singleton content family；noncommercial-safe terminal action；控制记录中的六槽 ordered pain-chain contract 可从自然 buyer-facing prose 唯一映射；buyer-task product-link anchor；全部 CTA transmission actions 与 exact route evidence；产品 claim ledger；stable producer/reviewer IDs 与 separation；四类 Production snapshot 的 kind-specific payload、freshness、provenance 和 byte-bound digest。

Fixture/example IDs、`example.test`、placeholder snapshot、synthetic identities 与测试 digest 只能记录结构验证，不能填作 Production proof。任何 unsupported outcome promise、空/过期/来源不明 snapshot、reviewer/producer 同人、未验证 route 的立即传递指令或未登记产品 claim 都令 `fatal_gate_verdict=block`。

## 9. CMS mutation and three-layer readback

`Published` is a lifecycle evidence conclusion, not a writable status string. Authorization, CMS mutation, backend readback, editor reopen, anonymous frontend, desktop, mobile, and image fetch/decode each require their own immutable artifact and digest, exact site/record/URL, stable producer and independent reviewer, `observed_at`, `reviewed_at`, and review ceiling. Missing, stale, cross-scope, digest-mismatched, same-actor, or late-reviewed evidence keeps the lifecycle BLOCK; toast or HTTP 200 cannot close an axis.


| Step | Expected | Actual | pass/block | Evidence path/time |
|---|---|---|---|---|
| Authorization revalidation | Exact site, operation, target, actor and unexpired time bound |  |  |  |
| API / Server Action | Expected operation response |  |  |  |
| Backend refresh / readback | Exact fields, IDs, lifecycle state and relations |  |  |  |
| Editor reopen | Body, hierarchy, links, CTA and formats persist |  |  |  |
| Anonymous frontend detail | Correct public page and status |  |  |  |
| Desktop acceptance | Hierarchy, scanning, links, images and CTA |  |  |  |
| Mobile acceptance | No blocking overflow or broken interaction; complex tables require real renderer/readability evidence at 320px before PASS |  |  |  |
| Image fetch / decode | Correct anonymous asset response and rendering |  |  |  |

Do not mark the mutation complete from an API response or toast alone. Use backend readback, editor reopen and anonymous frontend acceptance as applicable.

## 10. Result rows

| Item ID | Action | Status | CMS ID | URL | Verification | Error | Rollback |
|---|---|---|---|---|---|---|---|
|  | create / update / skip | success / failed / blocked |  |  |  |  |  |

## 11. Deferred frontend and renderer blocks

These remain independent deferred BLOCKs and are not closed by article-content PASS. Until all three are implemented and final-DOM verified, this record must not claim formal page SEO PASS:
...
## 12. Outcome measurement boundary

| Outcome | Allowed status enum | Evidence required for any non-`unverified` result |
|---|---|---|
| Ranking / organic visibility | `unverified|not-applicable|observed-no-improvement|observed-improvement` | Dated observation window, metric/event definition, observed result, versioned page/query/market evidence refs, accountable reviewer |
| Inquiry quantity / quality | `unverified|not-applicable|observed-no-improvement|observed-improvement` | Dated observation window, metric/event definition, observed result, qualification evidence refs, accountable reviewer |
| Sales acceptance | `unverified|not-applicable|not-accepted|sales-accepted` | Dated observation window, acceptance-event definition, observed result, evidence refs, accountable reviewer |
| Conversion / revenue | `unverified|not-applicable|observed-no-improvement|observed-improvement` | Dated observation window, metric/event definition, observed result, versioned funnel evidence refs, accountable reviewer |

Use `actual_outcome_observation_window`, `actual_outcome_metric_or_event_definition`, `actual_outcome_observed_result`, `actual_outcome_evidence_refs`, `actual_outcome_accountable_reviewer`, and `actual_sales_acceptance_evidence_refs` as the closed evidence contract. Any `observed-no-improvement`, `observed-improvement`, `not-accepted`, or `sales-accepted` result requires the dated window, defined metric/event, factual observed result, non-empty evidence refs, and identifiable accountable reviewer. `sales-accepted` additionally requires non-empty sales-acceptance evidence refs, affirmative evidence for all four canonical sales-acceptance gates, complete commercial inputs, and evidence that the named canonical commercial owner reviewed and accepted the opportunity. Without those conditions, keep the status `unverified` or `not-applicable` and make no outcome claim.

Neither this record nor a successful publish promises ranking, inquiries, sales acceptance, revenue or conversion lift.

## 13. Rollback

- Last-known-good artifact / content digest:
- Rollback trigger:
- Rollback steps:
...
- Skipped:
- Fatal gate verdict:
- Four-record exact-projection verdict:
- First-round output candidate gate verdict:
- Candidate decision gates satisfied: complete-second-round-package / named-technical-owner-review / not-applicable
- CTA route/policy transmission verdict:
- Structure score / verdict:
- Production evidence score / verdict:
- Release decision: blocked / ready-for-cms-draft / published
- Production readiness: block / pass
- Unexpected impact:
- Residual risk:
- Evidence refs:
- Writeback path:
