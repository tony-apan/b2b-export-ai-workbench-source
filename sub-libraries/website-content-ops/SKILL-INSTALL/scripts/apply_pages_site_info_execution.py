#!/usr/bin/env python3
"""Validate pages/site-info execution evidence and refresh source execution status."""
...
from validate_pages_site_info_execution_evidence import load_json as load_pages_json
from validate_pages_site_info_execution_evidence import site_info_field_count
from validate_pages_site_info_execution_evidence import validate_evidence as validate_pages_evidence
...
def artifact_paths(output_dir: Path) -> dict[str, Path]:
    return {
        "validation": output_dir / "pages-site-info-execution-validation.json",
        "source_status": output_dir / "source-execution-status.after-pages-site-info.json",
        "next_stage_handoff": output_dir / "source-next-stage-handoff.after-pages-site-info.json",
        "summary": output_dir / "pages-site-info-execution-apply-summary.json",
    }
...
def source_context_entries(args: argparse.Namespace, handoff: dict[str, Any], evidence: dict[str, Any]) -> list[tuple[str, dict[str, Any] | None]]:
    ...
    return [
        ...
        ("pages/site-info handoff", handoff),
        ("pages/site-info evidence", evidence),
    ]
...
def build(args: argparse.Namespace) -> dict[str, Any]:
    ...
    evidence = load_pages_json(Path(args.pages_site_info_evidence), "pages/site-info execution evidence")
    handoff = load_pages_json(Path(args.pages_site_info_handoff), "pages/site-info handoff")
    ...
    if context_issues:
        raise SystemExit("ERROR: pages/site-info source context invalid:\n- " + "\n- ".join(context_issues))
    issues = validate_pages_evidence(evidence, handoff)
    validation = {
        "kind": "allincms_pages_site_info_execution_evidence_validation",
        "generatedAt": now_iso(),
        "valid": not issues,
        "evidence": args.pages_site_info_evidence,
        "handoff": args.pages_site_info_handoff,
    ...
    if issues and args.fail_on_invalid:
        raise SystemExit("ERROR: pages/site-info execution evidence invalid:\n- " + "\n- ".join(issues))
    ...
    source_status = summarize_source_status(
        SimpleNamespace(
            ...
            pages_site_info_validation=str(paths["validation"]),
    ...
    ready_for_next = "blocked_pages_site_info_evidence" if issues else str(source_status.get("currentStage"))
    summary = {
        "kind": "allincms_pages_site_info_execution_apply_summary",
        "generatedAt": now_iso(),
        "localOnly": True,
        ...
        "siteInfoFieldCount": validation["siteInfoFieldCount"],
        ...
        **({"createdSiteSubmittedValues": submitted_values} if submitted_values else {}),
        ...
        "artifacts": {
            "pagesSiteInfoValidation": str(paths["validation"]),
            "sourceExecutionStatus": str(paths["source_status"]),
            "sourceNextStageHandoff": str(paths["next_stage_handoff"]),
        },
        "validation": {"pagesSiteInfoIssues": issues},
        "adversarialChecks": [
            "This helper validates redacted evidence and refreshes local status only; it does not mutate AllinCMS.",
            "Pages/site-info proof must include site-info persistence plus page create/save/publish/enable/route/frontend proof.",
            "A valid pages/site-info stage does not skip taxonomy, schema capture, sample upload, batch upload, launch QA, or cleanup gates.",
            "Follow sourceExecutionStatus.currentStage instead of assuming schema capture is next.",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Apply pages/site-info execution evidence to source execution status.")
    ...
    parser.add_argument("--pages-site-info-handoff", required=True)
    parser.add_argument("--pages-site-info-evidence", required=True)
    ...
    if args.json:
        ...
        print(f"Wrote pages/site-info execution apply summary: {summary['artifacts']['sourceExecutionStatus']}")
