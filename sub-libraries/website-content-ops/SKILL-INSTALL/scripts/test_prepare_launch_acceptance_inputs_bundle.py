...
def test_launch_acceptance_inputs_validation_requires_final_inputs() -> None:
    ...
    inputs = {
        ...
        "runEvidence": f"{tmp_dir}/run-evidence.json",
        "moduleCoverage": f"{tmp_dir}/module-coverage.json",
        "finalFrontendAudit": f"{tmp_dir}/final-frontend-audit.json",
        "cleanupEvidence": f"{tmp_dir}/cleanup-evidence.json",
        "formsMediaSettings": f"{tmp_dir}/forms-media-settings.json",
        "roundCloseout": "",
        "autoFinalCloseout": True,
        "finalCloseoutSedimentation": "updated",
        "finalCloseoutSedimentationNote": "Recorded final launch proof.",
        "uploadReadiness": [f"{tmp_dir}/upload-readiness.json"],
        "sampleEvidence": [f"{tmp_dir}/products-sample.json"],
        "batchValidation": [f"{tmp_dir}/products-batch-validation.json"],
        "package": f"{tmp_dir}/package.json",
        "confirmation": f"{tmp_dir}/confirmation.json",
        "executionPlan": f"{tmp_dir}/execution-plan.json",
        "artifactReadiness": f"{tmp_dir}/artifact-readiness.json",
        "createdSiteBinding": f"{tmp_dir}/created-site-binding.json",
    }
...
def test_launch_acceptance_inputs_validation_requires_per_content_type_sample_and_batch_paths() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        inputs = {
            "kind": "allincms_launch_acceptance_inputs",
            "runEvidence": f"{tempfile.gettempdir()}/run-evidence.json",
            "moduleCoverage": f"{tempfile.gettempdir()}/module-coverage.json",
            "finalFrontendAudit": f"{tempfile.gettempdir()}/final-frontend-audit.json",
            "cleanupEvidence": f"{tempfile.gettempdir()}/cleanup-evidence.json",
            "formsMediaSettings": f"{tempfile.gettempdir()}/forms-media-settings.json",
            "roundCloseout": "",
            "autoFinalCloseout": True,
            "finalCloseoutSedimentation": "updated",
            "finalCloseoutSedimentationNote": "Recorded final launch proof.",
            "uploadReadiness": [f"{tempfile.gettempdir()}/upload-readiness.json"],
            "sampleEvidence": [products_sample],
            "batchValidation": [products_batch],
            "package": f"{tempfile.gettempdir()}/package.json",
            "confirmation": f"{tempfile.gettempdir()}/confirmation.json",
            "executionPlan": f"{tempfile.gettempdir()}/execution-plan.json",
            "artifactReadiness": f"{tempfile.gettempdir()}/artifact-readiness.json",
            "createdSiteBinding": f"{tempfile.gettempdir()}/created-site-binding.json",
