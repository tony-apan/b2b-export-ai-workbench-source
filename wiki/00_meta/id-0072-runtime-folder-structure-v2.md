...
6. 迁移方式（待授权；TERRA 六条件全部满足后执行）：
   ...
   **警告 2（整树搬迁）**：运行区 11 个文件含 `701_kecheng/customer-runtime` **绝对路径**引用（sinopro-site-fast-run TASK.json/image-index/media-manifest、thermal-flask HANDOFF、internal-allincms-test 系列）——相对软链随母库整树走不断，但这 11 处绝对引用**在整树改名/搬迁后全断**；迁后应统一改 $RUNTIME_ROOT 或相对锚（与待办 RUNBOOK 路径约定同批）。
...
- [ ] 用户授权后执行迁移 + 软链（六条件已备：①gitignore 已修 ②整目录相对链 ③701_runtime 不存在先验 ④realpath 绝对路径验证 ⑤64 全知识升级分拣 ⑥skill commit）。**绑定条款（TERRA X3）：授权迁移 = 同时签字立项待办①**（interface-kit 真源管线：建任务卡+期限；不要求管线完工才准迁，但要求立项才准迁）
...
- [ ] .DS_Store 加 .gitignore；RUNBOOK-ANYONE:34 的"仓库根"路径约定随迁移改 $RUNTIME_ROOT
