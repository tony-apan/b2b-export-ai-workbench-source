...
| `OQ-REL-0003` | blocked | GitHub Protected Environment、required reviewers、ruleset、可信 signer 和正式 qualification 尚未建立。 | Tony | 母库公开/Stable + 子库 Stable qualification | 2026-09-07：`formal-release` environment + `FORMAL_RELEASE_TRUSTED_WORKFLOW_SHA`（锚定 1a05684）+ `FORMAL_RELEASE_ENVIRONMENT_READY` 已配置；本地 prepare→qualify、archive/checksum/attestation 合同与 55/55 攻击测试已闭合；母库 0.3.4 候选 Ready。仍缺：signer 公钥 secret、subkey 指纹 allowlist、本地 GPG 私钥签署、真人 approved_by、正式 tag/分支 ruleset；至今无任何 qualification 服务端 run | Tony 提供正式 signer 公钥（+私钥本地签 tag）与指纹 allowlist 并给出真人批准后，配置 secrets/ruleset、构造 approval intent、签名 `mother/v0.3.4` 并 dispatch 正式 qualification；候选保持 Ready 不提前 tag。 |
