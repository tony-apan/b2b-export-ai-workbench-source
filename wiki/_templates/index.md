---
title: "Templates Index"
description: "所有可复用模板的索引，覆盖来源、概念、playbook、客户、竞品、活动、内容、实验、发布和模块扩展。"
type: "index"
status: "Working"
owner: "AI"
created: "2026-06-28"
last_updated: "2026-06-28"
sources: ["User request"]
related: ["../00_meta/markdown-standard.md", "../00_meta/module-expansion-sop.md"]
visibility: "internal"
---

# Templates Index

## Knowledge Templates

- [source-note.md](source-note.md)
- [concept-page.md](concept-page.md)
- [playbook.md](playbook.md)
- [decision.md](decision.md)
- [conversation-note.md](conversation-note.md)
- [raw-extracted-markdown.md](raw-extracted-markdown.md)

## Business Templates

- [client-profile.md](client-profile.md)
- [competitor-profile.md](competitor-profile.md)
- [campaign-brief.md](campaign-brief.md)

## Channel Templates

- [linkedin-post.md](linkedin-post.md)
- [cold-email.md](cold-email.md)
- [seo-brief.md](seo-brief.md)
- [ads-test.md](ads-test.md)
- [sales-call-notes.md](sales-call-notes.md)
- [video-brief.md](video-brief.md)
- [image-brief.md](image-brief.md)

## Governance Templates

- [module-index.md](module-index.md)
- [skill-spec.md](skill-spec.md)
- [publication-review.md](publication-review.md)
- [experiment-record.md](experiment-record.md)
- [geo-test-log.md](geo-test-log.md)
