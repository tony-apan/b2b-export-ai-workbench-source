...
doc_id: "ID-0011"
description: "指导关键词研究、内容、技术 SEO 和 Search Console 复盘；没有真实站点数据时只形成待验证方案，不承诺排名。"
type: "playbook"
status: "Working"
owner: "AI"
created: "2026-06-28"
...
related: ["index.md", "../00_meta/knowledge-compounding-system.md", "../40_business/index.md", "../80_metrics/index.md", "../50_channels/seo/index.md", "id-0010-geo-ai-search.md"]
when_to_read: "制定 SEO 研究、内容 brief、技术检查或 Search Console 复盘任务时，按本 playbook 执行；没有站点数据时只能输出假设版。"
keywords: ["SEO playbook", "内容 brief", "关键词研究", "技术 SEO", "Search Console", "假设版"]
---
...
SEO 先按 Google 官方规则确保页面可抓取、可索引、可理解、对用户有价值，再用 Search Console、Analytics、竞品与研究工具发现需求、发布假设并验证。SEO 的核心产出不只是文章和排名，还包括搜索意图、客户语言、FAQ、异议、产品事实和 offer 判断。不要把工具指标当成 SEO 真相，也不要承诺“保证收录/排名/AI 引用”。
...
1. 明确要验证的业务问题：客户是谁、在搜什么、为什么购买、现有知识缺什么。
2. 读取业务底座：ICP、offer、messaging、proof、目标市场和产品资料。
3. 收集 query、SERP、客户问题、竞品页面和官方资料，登记来源与待验证项。
4. 建官方基线检查：抓取、索引、robots/noindex、canonical、sitemap、移动端、页面体验、schema。
5. 按 ICP、购买阶段和意图区分问题，不只看搜索量。
6. 把新发现先更新到搜索意图、FAQ、客户痛点、产品事实或内容 backlog。
7. 为一个明确假设建立页面：pillar、supporting、product/application 或 conversion page。
8. 每篇内容绑定证据、CTA、验证指标和预期知识回写位置。
9. 发布后用 Search Console / Analytics / CRM / 销售反馈复盘。
10. 把有效、无效和冲突结果回写到 business、concept、channel、metrics 和 open questions。
11. 定期刷新过时内容，记录 refresh date、知识变化和指标变化。

## Knowledge Capture Outputs

一次 SEO 研究至少检查是否新增或修正了：

- 搜索意图与客户原话。
- ICP 痛点、购买阶段和异议。
- 产品参数、应用、比较、认证、交付或服务事实。
- FAQ、术语和 topic map。
- 可验证的 claim、proof 和 CTA。
- 竞品差异、未知问题和反证。

这些内容分别回写到 `20_concepts`、`40_business`、`50_channels/seo`、`80_metrics` 和 `open-questions.md`。只新增文章而没有知识回写，任务不算完整。

## Customer Conversation To Search Intent

客户聊天、询盘、销售电话、客服记录和站内搜索属于 Voice of Customer 输入。它们能揭示真实语言和业务情境，但不能直接当成关键词清单。

执行顺序：

1. **私有保存**：完整聊天进入私有 raw，记录来源、时间、对象和授权边界。
2. **确认数据模式**：真实聊天留在私有 raw 或客户运行区，对外使用时移除姓名、邮箱、电话、订单和账号信息；虚拟演示无需脱敏，但必须标注虚构。
3. **提取意图元素**：识别触发事件、表面问题、深层痛点、当前替代方案、不行动成本、期待结果、异议和所需证据。
4. **判断阶段**：区分认识问题、学习方法、比较方案、判断适用性、降低风险和准备行动。
5. **生成 query hypothesis**：分别形成问题词、方法词、场景词、比较词、风险词和行动词，不把假设冒充真实 query。
6. **交叉验证**：检查其他聊天、Search Console、SERP、竞品、社区、站内搜索、CRM 和销售反馈。
7. **选择页面类型**：教程、产品、应用场景、对比、案例、FAQ 或落地页。
8. **补齐事实与证据**：确认公司真正能回答，禁止用 AI 编造产品能力、数据或案例。
9. **发布与观察**：记录 query、impression、click、CTR、询盘质量、异议和转化。
10. **知识回写**：更新 customer pain、search intent、ICP、messaging、offers、proof、SEO/GEO 和 open questions。

使用 [Customer Voice To Search Intent Template](../_templates/customer-voice-to-search-intent.md) 完成转换。

### Privacy And Evidence Gate

- 精确客户原话默认只留在私有 raw；没有授权不公开引用。
- 单条聊天只能生成 `Signal` 或 `Hypothesis`。
- 多次独立重复或与搜索/页面/销售数据相互印证后，才能提高置信度。
- 搜索量不能替代 ICP relevance 和 business value。
- 抓取、索引、排名、AI 引用和商业转化是不同环节，不能互相冒充完成。
...
- [../20_concepts/id-0001-search-intent.md](../20_concepts/id-0001-search-intent.md)
